...
############################# Head ############################
head_title: "Java DOC 比较 API - 比较 DOC 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 DOC 文件。分析 DOC 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较DOC文件"
description: "在 Java 中对两个以上的 DOC 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 DOC 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 DOC 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.doc")) {
            ...
            comparer.compare("C:\\result.doc"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.doc"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.doc")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 DOC 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 DOC 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-doc"
          title: "关于 DOC 文件格式"
          content: |
            带有 .doc 扩展名的文件表示由 Microsoft Word 或其他二进制文件格式的文字处理文档生成的文档。该扩展最初用于几个不同操作系统上的纯文本文档。它可以包含多种不同类型的数据，例如图像、格式化以及纯文本、图形、图表、嵌入对象、链接、页面、页面格式、打印设置等等。由于它为用户编写手册、提案、规范、简历、文章或任何类似文档提供了多种选择，因此该格式在各种文档中很受欢迎。 DOC 的更新版本是 DOCX，它基于 Office OpenXML，其规范是公开可用的。
