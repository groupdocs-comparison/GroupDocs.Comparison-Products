...
############################# Head ############################
head_title: "API di confronto Java DICOM: confronta i file DICOM per le differenze"
head_description: "Confronta e unisci file DICOM nelle applicazioni Java, J2EE, J2SE. Analizza il riepilogo delle differenze nel contenuto, nel testo e nello stile di file, immagini e formati di documenti DICOM."

############################# Header ############################
title: "Confronta i file DICOM in Java"
description: "Esegui un confronto riga per riga tra più di due file DICOM in Java. Recupera un elenco di differenze e salva i file confrontati in un singolo documento."
...
steps:
    enable: true
    title_left: "Passaggi per confrontare i file DICOM in Java"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) consente agli sviluppatori Java di confrontare facilmente i file DICOM all'interno delle loro applicazioni utilizzando poche righe di codice.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.dicom")) {
            ...
            comparer.compare("C:\\result.dicom"); // Crea il file dei risultati con il nome specificato
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.dicom"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.dicom")); // Crea il file dei risultati con il nome specificato
        }
        ```
...
demos:
    enable: true
    title: "Demo dal vivo per confrontare file DICOM"
    content: |
        Confronta subito i file DICOM visitando il sito web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          ...
          content: |
            DICOM è l'acronimo di Digital Imaging and Communications in Medicine e appartiene al campo dell'informatica medica. DICOM è la combinazione della definizione del formato file e di un protocollo di comunicazione di rete. DICOM utilizza l'estensione .DCM. .DCM esiste in due formati diversi, ovvero il formato 1.x e il formato 2.x. DCM Format 1.x è inoltre disponibile in due versioni, normale ed estesa. DICOM viene utilizzato per l'integrazione di dispositivi di imaging medico come stampanti, server, scanner, ecc. di vari fornitori e contiene anche dati identificativi di ciascun paziente per univocità. I file DICOM possono essere condivisi tra due parti se sono in grado di ricevere dati di immagine in formato DICOM. La parte di comunicazione di DICOM è un protocollo a livello di applicazione e utilizza TCP/IP per comunicare tra entità. Per i servizi web di DICOM vengono utilizzati i protocolli HTTP e HTTPS. Le versioni supportate dai servizi Web sono 1.0, 1.1, 2 o successive.
