...
############################# Head ############################
head_title: "Java ODP 比较 API - 比较 ODP 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 ODP 文件。分析 ODP 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较ODP文件"
description: "在 Java 中对两个以上的 ODP 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 ODP 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 ODP 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.odp")) {
            ...
            comparer.compare("C:\\result.odp"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.odp"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.odp")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 ODP 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 ODP 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-odp"
          title: "关于 ODP 文件格式"
          content: |
            带有 ODP 扩展名的文件表示 OpenOffice.org 在 OASISOpen 标准中使用的演示文件格式。演示文稿文件是幻灯片的集合，其中每张幻灯片都可以包含文本、图像、格式、动画和其他媒体。这些幻灯片以带有自定义演示设置的幻灯片形式呈现给观众。 ODP 文件可以由符合 OpenDocument 格式的应用程序（如 OpenOffice 或 StarOffice）打开。
