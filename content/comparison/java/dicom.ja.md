...
############################# Head ############################
head_title: "Java DICOM 比較 API-DICOM ファイルの違いを確認"
head_description: "Java、J2EE、J2SE アプリケーションの DICOM 個のファイルを比較してマージします。内容、テキスト、スタイルの違いをまとめて分析できます。"

############################# Header ############################
title: "Java 内の DICOM 個のファイルを比較" 
description: "Java 内の 2 つ以上の DICOM ファイル間のコンテンツ比較を実行します。相違点のリストを取得し、比較したファイルを 1 つの文書に保存します。"
...
steps:
    enable: true
    title: "Java を使用して複数の DICOM ドキュメントを比較する方法"
    content: |
      [GroupDocs.Comparison](https://products.groupdocs.com/comparison/java/) を使用して複数の DICOM ファイルを比較し、それらの違いを詳述したレポートを生成します
      ...
      2. Comparer クラスのインスタンスを作成し、DICOM ファイルのいずれかにパスを設定します
      3. Comparer インスタンスに少なくとも 1 つの DICOM を追加します
