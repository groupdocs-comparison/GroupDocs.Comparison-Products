...
############################# Head ############################
head_title: "Java VSS 비교 API - VSS 파일의 차이점 비교"
head_description: "Java, J2EE, J2SE 애플리케이션에서 VSS 파일을 비교하고 병합합니다. VSS 파일, 이미지 및 문서 형식의 내용, 텍스트 및 스타일의 차이점 요약을 분석합니다."

############################# Header ############################
title: "Java의 VSS 파일 비교"
description: "Java에서 두 개 이상의 VSS 파일을 한 줄씩 비교합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
steps:
    enable: true
    title_left: "Java에서 VSS 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/)을 사용하면 Java 개발자가 몇 줄의 코드를 사용하여 애플리케이션 내에서 VSS 파일을 쉽게 비교할 수 있습니다.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vss")) {
            ...
            comparer.compare("C:\\result.vss"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vss"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vss")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "VSS 파일 비교를 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 VSS 파일을 비교해 보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vss"
          title: "VSS 파일 형식 정보"
          content: |
            VSS는 Microsoft Visio 2007 및 이전 버전에서 생성된 스텐실 파일입니다. 비교적 새로운 파일 형식은 Microsoft Visio 2013에 도입된 .VSSX입니다. 스텐실 파일은 .VSD Visio 드로잉에 포함될 수 있는 드로잉 개체를 제공합니다. Microsoft Visio 자체는 모양 모음, 커넥터, 순서도, 네트워크 레이아웃, UML 다이어그램, 소프트웨어 다이어그램, 데이터베이스 모델, 개체 매핑 및 기타 유사한 정보와 같은 그리기 요소를 만드는 것으로 알려져 있습니다. 또한 Visio 문서를 PNG, BMP, PDF 등과 같은 다른 파일 형식으로 변환하는 풍부한 기능도 있습니다. Visio는 Windows와 Mac OS 모두에서 사용할 수 있습니다.
