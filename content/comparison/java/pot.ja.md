...
############################# Head ############################
head_title: "Java POT 比較 API - POT ファイルの相違点を比較"
head_description: "Java、J2EE、J2SE アプリケーションの POT ファイルを比較およびマージします。 POT ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を分析します。"

############################# Header ############################
title: "Java での POT ファイルの比較"
description: "Java で 2 つ以上の POT ファイル間の行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを 1 つのドキュメントに保存します。"
...
steps:
    enable: true
    title_left: "Java で POT ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) を使用すると、Java 開発者は数行のコードを使用してアプリケーション内の POT ファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.pot")) {
            ...
            comparer.compare("C:\\result.pot"); // 指定された名前で結果ファイルを作成します
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.pot"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.pot")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "POT ファイルを比較するためのライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ POT ファイルを比較してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-pot"
          title: "POT ファイル形式について"
          content: |
            拡張子が .POT のファイルは、PowerPoint 97 ～ 2003 バージョンで作成された Microsoft PowerPoint テンプレート ファイルを表します。これらのバージョンの Microsoft PowerPoint で作成されたファイルは、新しいバージョンの PowerPoint を使用して Office OpenXML ファイル形式で作成されたファイルと比較して、バイナリ形式です。したがって、生成されたファイルは、新しいファイルに適用するために必要な同じレイアウトやその他の設定を持つプレゼンテーションを作成するために使用できます。これらの設定には、スタイル、背景、カラー パレット、フォント、デフォルトが含まれます。このようなファイルは、公式に使用するためのすぐに使用できるテンプレート ファイルを作成するために生成されます。
