...
head_title: "JavaDIFF比較API-相違点についてDIFFファイルを比較します"
head_description: "Java、J2EE、J2SEアプリケーションのDIFFファイルを比較してマージします。コンテンツ、テキスト、およびテキストの違いの要約を分析します。 DIFFファイル、画像、ドキュメント形式のスタイル."

title: "JavaでDIFFファイルを比較する"
description: "Javaで3つ以上のDIFFファイル間で行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを1つのドキュメントに保存します."
...
steps:
    enable: true
    title_left: "JavaでDIFFファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/Comparison/java/)を使用すると、Java開発者は数行のコードを使用してアプリケーション内のDIFFファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.diff")) {
            ...
            comparer.compare("C:\\result.diff"); // 指定された名前の結果ファイルを作成する
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.diff"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.diff")); // 指定された名前の結果ファイルを作成する
        }
        ```
        
demos:
    enable: true
    title: "DIFFファイルを比較するためのライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、今すぐDIFFファイルを比較してください。  
        ライブデモには次の利点があります
