...
############################# Head ############################
head_title: "Java VSD 비교 API - VSD 파일의 차이점 비교"
head_description: "Java, J2EE, J2SE 애플리케이션에서 VSD 파일을 비교하고 병합합니다. VSD 파일, 이미지 및 문서 형식의 내용, 텍스트 및 스타일의 차이점 요약을 분석합니다."

############################# Header ############################
title: "Java의 VSD 파일 비교"
description: "Java에서 두 개 이상의 VSD 파일을 한 줄씩 비교합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
steps:
    enable: true
    title_left: "Java에서 VSD 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/)을 사용하면 Java 개발자가 몇 줄의 코드를 사용하여 애플리케이션 내에서 VSD 파일을 쉽게 비교할 수 있습니다.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vsd")) {
            ...
            comparer.compare("C:\\result.vsd"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vsd"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vsd")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "VSD 파일 비교를 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 VSD 파일을 비교해 보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vsd"
          title: "VSD 파일 형식 정보"
          content: |
            VSD 파일은 다양한 그래픽 개체와 이들 간의 상호 연결을 나타내기 위해 Microsoft Visio 응용 프로그램으로 만든 그림입니다. 이러한 도면에는 시각적 개체, 흐름도, UML 다이어그램, 정보 흐름, 조직도, 소프트웨어 다이어그램, 네트워크 레이아웃, 데이터베이스 모델, 개체 매핑 및 기타 유사한 정보와 같은 시각적 개체가 포함될 수 있습니다. Microsoft Visio는 Visio 파일을 PNG, BMP, PDF 등을 포함한 다양한 파일 형식으로 변환하는 기능을 제공합니다.
