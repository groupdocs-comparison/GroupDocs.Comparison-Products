...
############################# Head ############################
head_title: "Java DJVU 比较 API - 比较 DJVU 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 DJVU 文件。分析 DJVU 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较DJVU文件"
description: "在 Java 中对两个以上的 DJVU 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 DJVU 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 DJVU 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.djvu")) {
            ...
            comparer.compare("C:\\result.djvu"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.djvu"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.djvu")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 DJVU 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 DJVU 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-djvu"
          title: "关于 DJVU 文件格式"
          content: |
            DjVu，发音为"déjà vu"，是一种图形文件格式，适用于扫描的文档和书籍，尤其是那些包含文本、绘图、图像和照片的组合。它由 AT&T 实验室开发。它使用多种技术，如文本和背景图像的图像层分离、渐进加载、算术编码和双色图像的有损压缩。由于 DJVU 文件可以包含压缩但高质量的彩色图像、照片、文本和绘图，并且可以节省空间，因此它在网络上用作电子书、手册、报纸、古代文档等。
