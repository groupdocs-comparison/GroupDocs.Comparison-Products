...
############################# Head ############################
head_title: "Java JAVA 比較 API - JAVA ファイルの相違点を比較"
head_description: "Java、J2EE、J2SE アプリケーションの JAVA ファイルを比較およびマージします。 JAVA ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を分析します。"

############################# Header ############################
title: "Java での JAVA ファイルの比較"
description: "Java で 2 つ以上の JAVA ファイル間の行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを 1 つのドキュメントに保存します。"
...
steps:
    enable: true
    title_left: "Java で JAVA ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) を使用すると、Java 開発者は数行のコードを使用してアプリケーション内の JAVA ファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.java")) {
            ...
            comparer.compare("C:\\result.java"); // 指定された名前で結果ファイルを作成します
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.java"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.java")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "JAVA ファイルを比較するためのライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ JAVA ファイルを比較してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-java"
          title: "JAVA ファイル形式について"
          content: |
            Java は、複数のプラットフォームで実行できる汎用オブジェクト指向言語です。 Java ソース コードは、拡張子 .java が付いたファイルに含まれています。 Java は、モバイル アプリケーション、Web アプリケーション、デスクトップ アプリケーション、ゲームなどの開発に使用されます。Java は、Windows、Mac、Linux、Raspberry Pi などの複数のプラットフォームで動作します。Java は C# や C++ に非常に似ているため、これらの言語を切り替えます。
