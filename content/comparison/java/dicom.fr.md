...
############################# Head ############################
head_title: "API de comparaison Java DICOM - Comparez les fichiers DICOM pour les différences"
head_description: "Comparez et fusionnez des fichiers DICOM dans des applications Java, J2EE, J2SE. Analysez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents DICOM."

############################# Header ############################
title: "Comparez les fichiers DICOM en Java"
description: "Effectuez une comparaison ligne par ligne entre plus de deux fichiers DICOM en Java. Récupérez une liste des différences et enregistrez les fichiers comparés dans un seul document."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers DICOM en Java"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) permet aux développeurs Java de comparer facilement les fichiers DICOM au sein de leurs applications à l'aide de quelques lignes de code.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.dicom")) {
            ...
            comparer.compare("C:\\result.dicom"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.dicom"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.dicom")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démos en direct pour comparer les fichiers DICOM"
    content: |
        Comparez dès maintenant les fichiers DICOM en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          ...
          content: |
            DICOM est l'acronyme de Digital Imaging and Communications in Medicine et concerne le domaine de l'informatique médicale. DICOM est la combinaison d'une définition de format de fichier et d'un protocole de communication réseau. DICOM utilise l'extension .DCM. .DCM existe dans deux formats différents, à savoir le format 1.x et le format 2.x. DCM Format 1.x est en outre disponible en deux versions normale et étendue. DICOM est utilisé pour l'intégration de dispositifs d'imagerie médicale tels que des imprimantes, des serveurs, des scanners, etc. provenant de divers fournisseurs et contient également des données d'identification de chaque patient pour plus d'unicité. Les fichiers DICOM peuvent être partagés entre deux parties si elles sont capables de recevoir des données d'image au format DICOM. La partie communication de DICOM est un protocole de couche application et utilise TCP/IP pour communiquer entre les entités. Les protocoles HTTP et HTTPS sont utilisés pour les services Web de DICOM. Les versions prises en charge par les services Web sont 1.0, 1.1, 2 ou ultérieures.
