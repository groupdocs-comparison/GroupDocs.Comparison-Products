...
############################# Head ############################
head_title: "Java JPG 比較 API - 比較 JPG 文件的差異"
head_description: "比較並合併 Java、J2EE、J2SE 應用程序中的 JPG 文件。分析 JPG 文件、圖像和文檔格式的內容、文本和風格的差異摘要。"

############################# Header ############################
title: "在 Java 中比較 JPG 文件"
description: "在 Java 中對兩個以上的 JPG 文件進行逐行比較。檢索差異列表並將比較的文件保存到單個文檔中。"
...
steps:
    enable: true
    title_left: "在 Java 中比較 JPG 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 開發人員可以使用幾行代碼輕鬆比較應用程序中的 JPG 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.jpg")) {
            ...
            comparer.compare("C:\\result.jpg"); // 創建指定名稱的結果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.jpg"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.jpg")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 JPG 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站比較 JPG 文件。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-jpg"
          title: "關於 JPG 文件格式"
          content: |
            JPEG 是一種使用有損壓縮方法保存的圖像格式。壓縮後的輸出圖像是存儲大小和圖像質量之間的權衡。用戶可以調整壓縮級別以達到所需的質量級別，同時減少存儲大小。如果對圖像應用 10:1 壓縮，圖像質量的影響可以忽略不計。壓縮值越高，圖像質量下降得越嚴重。
