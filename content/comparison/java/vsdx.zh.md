...
############################# Head ############################
head_title: "Java VSDX 比较 API - 比较 VSDX 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 VSDX 文件。分析 VSDX 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较VSDX文件"
description: "在 Java 中对两个以上的 VSDX 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 VSDX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 VSDX 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vsdx")) {
            ...
            comparer.compare("C:\\result.vsdx"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vsdx"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vsdx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 VSDX 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 VSDX 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vsdx"
          title: "关于 VSDX 文件格式"
          content: |
            带有 .VSDX 扩展名的文件代表从 Microsoft Office 2013 开始引入的 Microsoft Visio 文件格式。开发它是为了替换二进制文件格式 .VSD，后者受早期版本的 Microsoft Visio 支持。 Microsoft SharePoint Server 2013 中的 Visio Services 也支持它，并且不需要中间文件格式即可发布到 SharePoint Server。 Visio 文件用于创建包含可视对象、流程图、UML 图、信息流、组织结构图、软件图、网络布局、数据库模型、对象映射和其他类似信息的绘图。使用 Visio 生成的文件也可以导出为不同的文件格式，例如 PNG、BMP、PDF 等。
