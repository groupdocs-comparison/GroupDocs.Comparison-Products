...
############################# Head ############################
head_title: "API сравнения DICOM Java – сравнение файлов DICOM на наличие различий"
head_description: "Сравнивайте и объединяйте файлы DICOM в приложениях Java, J2EE, J2SE. Анализируйте сводную информацию о различиях в содержании, тексте и стиле DICOM файлов, изображений и форматов документов."

############################# Header ############################
title: "Сравнить файлы DICOM в Java"
description: "Выполните построчное сравнение между более чем двумя файлами DICOM в Java. Получите список различий и сохраните сравниваемые файлы в один документ."
...
steps:
    enable: true
    title_left: "Шаги для сравнения файлов DICOM в Java"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) позволяет разработчикам Java легко сравнивать файлы DICOM в своих приложениях, используя несколько строк кода.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.dicom")) {
            ...
            comparer.compare("C:\\result.dicom"); // Создать файл результатов с указанным именем
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.dicom"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.dicom")); // Создать файл результатов с указанным именем
        }
        ```
...
demos:
    enable: true
    title: "Живые демонстрации для сравнения файлов DICOM"
    content: |
        Сравните файлы DICOM прямо сейчас, посетив веб-сайт [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          ...
          content: |
            DICOM — это аббревиатура от Digital Imaging and Communications in Medicine, относящаяся к области медицинской информатики. DICOM — это сочетание определения формата файла и протокола сетевой связи. DICOM использует расширение .DCM. .DCM существует в двух разных форматах: формате 1.x и формате 2.x. DCM Format 1.x доступен в двух версиях: обычной и расширенной. DICOM используется для интеграции устройств медицинской визуализации, таких как принтеры, серверы, сканеры и т. д. от различных производителей, а также содержит идентификационные данные каждого пациента для обеспечения уникальности. Файлы DICOM могут использоваться совместно двумя сторонами, если они способны принимать данные изображения в формате DICOM. Коммуникационная часть DICOM представляет собой протокол прикладного уровня и использует TCP/IP для связи между объектами. Протоколы HTTP и HTTPS используются для веб-служб DICOM. Веб-службы поддерживают версии 1.0, 1.1, 2 или более поздние.
