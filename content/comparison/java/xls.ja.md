...
head_title: "JavaXLS比較API-XLSファイルの違いを比較します"
head_description: "Java、J2EE、J2SEアプリケーションのXLSファイルを比較してマージします。コンテンツ、テキスト、およびテキストの違いの要約を分析します。 XLSファイル、画像、ドキュメント形式のスタイル."

title: "JavaでXLSファイルを比較する"
description: "Javaで3つ以上のXLSファイル間で行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを1つのドキュメントに保存します."
...
steps:
    enable: true
    title_left: "JavaでXLSファイルを比較するための手順"
    content_left: |
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.xls")) {
            ...
            comparer.compare("C:\\result.xls"); // 指定された名前の結果ファイルを作成する
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.xls"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.xls")); // 指定された名前の結果ファイルを作成する
        }
        ```
        
demos:
    enable: true
    title: "XLSファイルを比較するためのライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、XLSファイルを今すぐ比較してください。  
        ライブデモには次の利点があります
