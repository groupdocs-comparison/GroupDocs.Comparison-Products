...
############################# Head ############################
head_title: "Java JAVA 比較 API - 比較 JAVA 文件的差異"
head_description: "比較並合併 Java、J2EE、J2SE 應用程序中的 JAVA 文件。分析 JAVA 文件、圖像和文檔格式的內容、文本和風格的差異摘要。"

############################# Header ############################
title: "在 Java 中比較 JAVA 文件"
description: "在 Java 中對兩個以上的 JAVA 文件進行逐行比較。檢索差異列表並將比較的文件保存到單個文檔中。"
...
steps:
    enable: true
    title_left: "在 Java 中比較 JAVA 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 開發人員可以使用幾行代碼輕鬆比較應用程序中的 JAVA 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.java")) {
            ...
            comparer.compare("C:\\result.java"); // 創建指定名稱的結果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.java"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.java")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 JAVA 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站比較 JAVA 文件。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-java"
          title: "關於 JAVA 文件格式"
          content: |
            Java 是一種通用的面向對象語言，可以在多個平台上運行。 Java 源代碼包含在擴展名為 .java 的文件中。 Java 用於開發移動應用程序、Web 應用程序、桌面應用程序、遊戲等。Java 可在多種平台上運行，包括 Windows、Mac、Linux、Raspberry Pi 等。Java 與 C# 和 C++ 非常相似，因此更容易在這些語言之間切換。
