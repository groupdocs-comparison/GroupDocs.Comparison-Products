...
############################# Head ############################
head_title: "Java JAVA 比较 API - 比较 JAVA 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 JAVA 文件。分析 JAVA 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较JAVA文件"
description: "在 Java 中对两个以上的 JAVA 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 JAVA 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 JAVA 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.java")) {
            ...
            comparer.compare("C:\\result.java"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.java"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.java")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 JAVA 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 JAVA 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-java"
          title: "关于 JAVA 文件格式"
          content: |
            Java 是一种通用的面向对象的语言，可以在多个平台上运行。 Java 源代码包含在扩展名为 .java 的文件中。 Java用于开发移动应用程序、Web应用程序、桌面应用程序、游戏等。Java适用于多种平台，包括Windows、Mac、Linux、Raspberry Pi等。Java与C#和C++非常相似，因此更容易在这些语言之间切换。
