...
############################# Head ############################
head_title: "Java DICOM 比较 API - 比较 DICOM 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 DICOM 文件。分析 DICOM 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较DICOM文件"
description: "在 Java 中对两个以上的 DICOM 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 DICOM 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 DICOM 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.dicom")) {
            ...
            comparer.compare("C:\\result.dicom"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.dicom"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.dicom")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 DICOM 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 DICOM 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dicom"
          title: "关于 DICOM 文件格式"
          content: |
            DICOM 是 Digital Imaging and Communications in Medicine 的首字母缩写，属于医学信息学领域。 DICOM 是文件格式定义和网络通信协议的结合。 DICOM 使用 .DCM 扩展名。 .DCM 以两种不同的格式存在，即格式 1.x 和格式 2.x。 DCM 格式 1.x 进一步提供了两个版本的正常版本和扩展版本。 DICOM 用于集成来自不同供应商的打印机、服务器、扫描仪等医疗成像设备，还包含每个患者的唯一性识别数据。如果双方能够接收 DICOM 格式的图像数据，则可以在双方之间共享 DICOM 文件。 DICOM的通信部分是应用层协议，实体间使用TCP/IP进行通信。 HTTP 和 HTTPS 协议用于 DICOM 的 Web 服务。 Web 服务支持的版本为 1.0、1.1、2 或更高版本。
