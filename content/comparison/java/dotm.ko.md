...
description: "Java에서 2개 이상의 DOTM 파일 간에 라인별로 비교를 수행합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
demos:
    enable: true
    title: "DOTM 파일을 비교하기 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DOTM 파일을 비교하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dotm"
          title: "DOTM 파일 형식 정보"
          content: |
            DOTM 확장자를 가진 파일은 Microsoft Word 2007 이상에서 만든 템플릿 파일을 나타냅니다. 새 문서를 만들 때 재사용할 수 있도록 사용자 정의 설정을 유지한다는 점을 제외하면 널리 사용되는 DOCX 파일 형식과 유사합니다. 이러한 문서는 페이지 정보, 여백, 기본 레이아웃 및 매크로와 같은 설정으로 표준 템플릿 파일이 생성되고 필요할 때 새 문서를 만드는 데 사용되는 사무실에서 더 자주 사용됩니다. 그러나 DOTM 파일은 작업의 자동 완료를 위해 기록된 동작 형태의 일련의 명령인 매크로를 저장합니다. 이렇게 하면 작업을 완료할 때 반복되는 작업을 수행할 때 시간을 절약할 수 있습니다.
