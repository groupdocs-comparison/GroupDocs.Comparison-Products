...
############################# Head ############################
head_title: "Java DOCM 比较 API - 比较 DOCM 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 DOCM 文件。分析 DOCM 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较DOCM文件"
description: "在 Java 中对两个以上的 DOCM 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 DOCM 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 DOCM 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.docm")) {
            ...
            comparer.compare("C:\\result.docm"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.docm"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.docm")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 DOCM 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 DOCM 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-docm"
          title: "关于 DOCM 文件格式"
          content: |
            DOCM 文件是 Microsoft Word 2007 或更高版本生成的文档，能够运行宏。它类似于 DOCX 文件格式，但运行宏的能力使其不同于 DOCX。与DOCX一样，DOCM文件可以存储文本、图像、表格、形状、图表等内容。运行宏的能力可以通过执行一系列命令以记录动作的形式自动完成任务，从而轻松节省时间. DOCM 文件可以在 Microsoft Word 2007 及更高版本中打开和编辑。
