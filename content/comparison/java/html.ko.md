...
############################# Head ############################
head_title: "Java HTML 비교 API - HTML 파일의 차이점 비교"
head_description: "Java, J2EE, J2SE 애플리케이션에서 HTML 파일을 비교하고 병합합니다. HTML 파일, 이미지 및 문서 형식의 내용, 텍스트 및 스타일의 차이점 요약을 분석합니다."

############################# Header ############################
title: "Java의 HTML 파일 비교"
description: "Java에서 두 개 이상의 HTML 파일을 한 줄씩 비교합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
steps:
    enable: true
    title_left: "Java에서 HTML 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/)을 사용하면 Java 개발자가 몇 줄의 코드를 사용하여 애플리케이션 내에서 HTML 파일을 쉽게 비교할 수 있습니다.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.html")) {
            ...
            comparer.compare("C:\\result.html"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.html"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.html")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "HTML 파일 비교를 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 HTML 파일을 비교해 보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-html"
          title: "HTML 파일 형식 정보"
          content: |
            HTML(Hyper Text Markup Language)은 브라우저에 표시하기 위해 만들어진 웹 페이지의 확장입니다. 웹 언어로 알려진 HTML은 웹 페이지의 일부로 표시되어야 하는 새로운 정보 요구 사항에 따라 발전해 왔습니다. 최신 변형은 언어 작업에 많은 유연성을 제공하는 HTML 5로 알려져 있습니다. HTML 페이지는 호스팅되는 서버에서 수신되거나 로컬 시스템에서도 로드될 수 있습니다. 각 HTML 페이지는 양식, 텍스트, 이미지, 애니메이션, 링크 등과 같은 HTML 요소로 구성됩니다. 이러한 요소는 img, a, p 및 각 태그의 시작과 끝이 있는 기타 여러 태그로 표시됩니다. 또한 전체 레이아웃 표현을 위해 JavaScript 및 스타일 시트(CSS)와 같은 스크립팅 언어로 작성된 애플리케이션을 내장할 수도 있습니다.
