...
############################# Head ############################
head_title: "Java VSS 比较 API - 比较 VSS 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 VSS 文件。分析 VSS 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较VSS文件"
description: "在 Java 中对两个以上的 VSS 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 VSS 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 VSS 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vss")) {
            ...
            comparer.compare("C:\\result.vss"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vss"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vss")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 VSS 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 VSS 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vss"
          title: "关于 VSS 文件格式"
          content: |
            VSS 是使用 Microsoft Visio 2007 及更早版本创建的模板文件。一种相对较新的文件格式是随 Microsoft Visio 2013 引入的 .VSSX。Stencil 文件提供可以包含在 .VSD Visio 绘图中的绘图对象。 Microsoft Visio 本身以创建绘图元素而闻名，例如形状集合、连接器、流程图、网络布局、UML 图、软件图、数据库模型、对象映射和其他类似信息。它还具有将 Visio 文档转换为其他文件格式（如 PNG、BMP、PDF 等）的丰富功能。 Visio 适用于 Windows 和 Mac 操作系统。
