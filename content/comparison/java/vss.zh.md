...
############################# Head ############################
head_title: "Java VSS 比較 API - 比較 VSS 文件的差異"
head_description: "比較並合併 Java、J2EE、J2SE 應用程序中的 VSS 文件。分析 VSS 文件、圖像和文檔格式的內容、文本和風格的差異摘要。"

############################# Header ############################
title: "在 Java 中比較 VSS 文件"
description: "在 Java 中對兩個以上的 VSS 文件進行逐行比較。檢索差異列表並將比較的文件保存到單個文檔中。"
...
steps:
    enable: true
    title_left: "在 Java 中比較 VSS 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 開發人員可以使用幾行代碼輕鬆比較應用程序中的 VSS 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vss")) {
            ...
            comparer.compare("C:\\result.vss"); // 創建指定名稱的結果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vss"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vss")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 VSS 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站比較 VSS 文件。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vss"
          title: "關於 VSS 文件格式"
          content: |
            VSS 是使用 Microsoft Visio 2007 及更早版本創建的模板文件。相對較新的文件格式是 Microsoft Visio 2013 中引入的 .VSSX。模板文件提供可包含在 .VSD Visio 繪圖中的繪圖對象。 Microsoft Visio 本身以創建繪圖元素而聞名，例如形狀、連接器、流程圖、網絡佈局、UML 圖、軟件圖表、數據庫模型、對象映射和其他類似信息的集合。它還具有豐富的 Visio 文檔到其他文件格式（如 PNG、BMP、PDF 等）的轉換功能。 Visio 可用於 Windows 和 Mac 操作系統。
