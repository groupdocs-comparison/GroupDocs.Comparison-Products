...
############################# Head ############################
head_title: "Java MOBI 比較 API - MOBI ファイルの相違点を比較"
head_description: "Java、J2EE、J2SE アプリケーションの MOBI ファイルを比較およびマージします。 MOBI ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を分析します。"

############################# Header ############################
title: "Java での MOBI ファイルの比較"
description: "Java で 2 つ以上の MOBI ファイル間の行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを 1 つのドキュメントに保存します。"
...
steps:
    enable: true
    title_left: "Java で MOBI ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) を使用すると、Java 開発者は数行のコードを使用してアプリケーション内の MOBI ファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.mobi")) {
            ...
            comparer.compare("C:\\result.mobi"); // 指定された名前で結果ファイルを作成します
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.mobi"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.mobi")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "MOBI ファイルを比較するためのライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ MOBI ファイルを比較してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-mobi"
          title: "MOBI ファイル形式について"
          content: |
            MOBI ファイル形式は、最も広く使用されている電子書籍ファイル形式の 1 つです。この形式は古い OEB (Open Ebook Format) 形式を拡張したもので、Mobipocket Reader の独自形式として使用されていました。 EPUB と同様に、ほとんどすべての最新の電子書籍リーダー、特に低帯域幅のモバイル デバイスでサポートされています。この形式は、Kindle アプリなどの公的に入手可能なソフトウェア アプリケーションを使用して、PDF、EPUB、その他のいくつかの形式に変換できます。 Project Gutenberg、Feedbooks、Open Library など、無料の MOBI 書籍を提供する会社がいくつかあります。
