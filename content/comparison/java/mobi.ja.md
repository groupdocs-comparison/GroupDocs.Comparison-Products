...
############################# Head ############################
head_title: "JavaMOBI比較API-MOBIファイルの違いを比較する"
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/java/)を使用すると、Java開発者は数行のコードを使用してアプリケーション内のMOBIファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.mobi")) {
            ...
            comparer.compare("C:\\result.mobi"); // 指定された名前の結果ファイルを作成する
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.mobi"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.mobi")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    ...
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、MOBIファイルを今すぐ比較してください。  
        ライブデモには次の利点があります
