...
############################# Head ############################
head_title: "Java ODP 비교 API - ODP 파일의 차이점 비교"
head_description: "Java, J2EE, J2SE 애플리케이션에서 ODP 파일을 비교하고 병합합니다. ODP 파일, 이미지 및 문서 형식의 내용, 텍스트 및 스타일의 차이점 요약을 분석합니다."

############################# Header ############################
title: "Java의 ODP 파일 비교"
description: "Java에서 두 개 이상의 ODP 파일을 한 줄씩 비교합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
steps:
    enable: true
    title_left: "Java에서 ODP 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/)을 사용하면 Java 개발자가 몇 줄의 코드를 사용하여 애플리케이션 내에서 ODP 파일을 쉽게 비교할 수 있습니다.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.odp")) {
            ...
            comparer.compare("C:\\result.odp"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.odp"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.odp")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "ODP 파일 비교를 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 ODP 파일을 비교해 보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-odp"
          title: "ODP 파일 형식 정보"
          content: |
            ODP 확장자를 가진 파일은 OASISOpen 표준의 OpenOffice.org에서 사용되는 프레젠테이션 파일 형식을 나타냅니다. 프리젠테이션 파일은 각 슬라이드가 텍스트, 이미지, 서식, 애니메이션 및 기타 미디어로 구성될 수 있는 슬라이드 모음입니다. 이러한 슬라이드는 사용자 정의 프레젠테이션 설정을 사용하여 슬라이드쇼 형식으로 청중에게 제공됩니다. ODP 파일은 OpenDocument 형식(예: OpenOffice 또는 StarOffice)을 준수하는 응용 프로그램에서 열 수 있습니다.
