...
############################# Head ############################
head_title: "Java VSD 比较 API - 比较 VSD 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 VSD 文件。分析 VSD 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较VSD文件"
description: "在 Java 中对两个以上的 VSD 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 VSD 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 VSD 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.vsd")) {
            ...
            comparer.compare("C:\\result.vsd"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.vsd"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.vsd")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 VSD 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 VSD 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vsd"
          title: "关于 VSD 文件格式"
          content: |
            VSD 文件是使用 Microsoft Visio 应用程序创建的图形，用于表示各种图形对象以及这些对象之间的互连。此类绘图可以包含可视对象，例如可视对象、流程图、UML 图、信息流、组织结构图、软件图、网络布局、数据库模型、对象映射和其他类似信息。 Microsoft Visio 提供将 Visio 文件转换为多种不同文件格式的功能，包括 PNG、BMP、PDF 等。
