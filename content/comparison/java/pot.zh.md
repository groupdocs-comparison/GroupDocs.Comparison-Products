...
############################# Head ############################
head_title: "Java POT 比较 API - 比较 POT 文件的差异"
head_description: "比较和合并 Java、J2EE、J2SE 应用程序中的 POT 文件。分析 POT 文件、图像和文档格式在内容、文本和样式方面的差异摘要。"

############################# Header ############################
title: "在Java中比较POT文件"
description: "在 Java 中对两个以上的 POT 文件进行逐行比较。检索差异列表并将比较的文件保存到单个文档中。"
...
steps:
    enable: true
    title_left: "在 Java 中比较 POT 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 开发人员可以使用几行代码轻松比较其应用程序中的 POT 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.pot")) {
            ...
            comparer.compare("C:\\result.pot"); // 创建指定名称的结果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.pot"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.pot")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 POT 文件的现场演示"
    content: |
        立即访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站比较 POT 文件。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-pot"
          title: "关于 POT 文件格式"
          content: |
            带有 .POT 扩展名的文件代表由 PowerPoint 97-2003 版本创建的 Microsoft PowerPoint 模板文件。与使用更高版本的 PowerPoint 以 Office OpenXML 文件格式创建的文件相比，使用这些版本的 Microsoft PowerPoint 创建的文件采用二进制格式。因此，生成的文件可用于创建具有相同布局和其他需要应用于新文件的设置的演示文稿。这些设置可以包括样式、背景、调色板、字体和默认设置。生成此类文件是为了创建供官方使用的即用型模板文件。
