...
############################# Head ############################
head_title: "Java DXF 比較 API - 比較 DXF 文件的差異"
head_description: "比較並合併 Java、J2EE、J2SE 應用程序中的 DXF 文件。分析 DXF 文件、圖像和文檔格式的內容、文本和風格的差異摘要。"

############################# Header ############################
title: "在 Java 中比較 DXF 文件"
description: "在 Java 中對兩個以上的 DXF 文件進行逐行比較。檢索差異列表並將比較的文件保存到單個文檔中。"
...
steps:
    enable: true
    title_left: "在 Java 中比較 DXF 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) 使 Java 開發人員可以使用幾行代碼輕鬆比較應用程序中的 DXF 文件。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.dxf")) {
            ...
            comparer.compare("C:\\result.dxf"); // 創建指定名稱的結果文件
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.dxf"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.dxf")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    ...
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站比較 DXF 文件。
        現場演示有以下好處
