...
description: "Java에서 2개 이상의 DOC 파일 간에 라인별로 비교를 수행합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
demos:
    enable: true
    title: "DOC 파일을 비교하기 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DOC 파일을 비교하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-doc"
          title: "DOC 파일 형식 정보"
          content: |
            확장자가 .doc인 파일은 Microsoft Word에서 생성된 문서 또는 이진 파일 형식의 기타 워드 프로세싱 문서를 나타냅니다. 확장은 처음에 여러 다른 운영 체제에서 일반 텍스트 문서에 사용되었습니다. 여기에는 이미지, 일반 텍스트, 그래프, 차트, 포함된 개체, 링크, 페이지, 페이지 서식, 인쇄 설정 등과 같은 여러 가지 유형의 데이터가 포함될 수 있습니다. 이 형식은 설명서, 제안서, 사양서, 이력서, 기사 또는 유사한 문서를 작성하기 위해 사용자에게 제공하는 다양한 옵션으로 인해 모든 종류의 문서에 널리 사용되었습니다. 업데이트된 DOC 버전은 사양이 공개된 Office OpenXML을 기반으로 하는 DOCX입니다.
