...
############################# Head ############################
head_title: "Java DJVU 比較 API - DJVU ファイルの相違点を比較"
head_description: "Java、J2EE、J2SE アプリケーションの DJVU ファイルを比較およびマージします。 DJVU ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を分析します。"

############################# Header ############################
title: "Java での DJVU ファイルの比較"
description: "Java で 2 つ以上の DJVU ファイル間の行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを 1 つのドキュメントに保存します。"
...
steps:
    enable: true
    title_left: "Java で DJVU ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) を使用すると、Java 開発者は数行のコードを使用してアプリケーション内の DJVU ファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.djvu")) {
            ...
            comparer.compare("C:\\result.djvu"); // 指定された名前で結果ファイルを作成します
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.djvu"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.djvu")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "DJVU ファイルを比較するためのライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ DJVU ファイルを比較してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-djvu"
          ...
          content: |
            「デジャブ」と発音される DjVu は、スキャンされたドキュメントや書籍、特にテキスト、描画、画像、写真の組み合わせを含むものを対象としたグラフィック ファイル形式です。 AT&T Labs によって開発されました。テキストと背景画像の画像層分離、プログレッシブ読み込み、算術符号化、モノクロ画像の非可逆圧縮などの複数の技術を使用します。 DJVU ファイルは、圧縮されながらも高品質のカラー画像、写真、テキスト、図面を含めることができ、より少ないスペースで保存できるため、Web 上で電子書籍、マニュアル、新聞、古文書などとして使用されます。
