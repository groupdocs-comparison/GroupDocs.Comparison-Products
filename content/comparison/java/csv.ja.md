...
############################# Head ############################
head_title: "Java CSV 比較 API - CSV ファイルの相違点を比較"
head_description: "Java、J2EE、J2SE アプリケーションの CSV ファイルを比較およびマージします。 CSV ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を分析します。"

############################# Header ############################
title: "Java での CSV ファイルの比較"
description: "Java で 2 つ以上の CSV ファイル間の行ごとの比較を実行します。相違点のリストを取得し、比較したファイルを 1 つのドキュメントに保存します。"
...
steps:
    enable: true
    title_left: "Java で CSV ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/) を使用すると、Java 開発者は数行のコードを使用してアプリケーション内の CSV ファイルを簡単に比較できます。
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.csv")) {
            ...
            comparer.compare("C:\\result.csv"); // 指定された名前で結果ファイルを作成します
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.csv"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.csv")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "CSV ファイルを比較するためのライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ CSV ファイルを比較してください。
