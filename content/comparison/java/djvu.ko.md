...
############################# Head ############################
head_title: "Java DJVU 비교 API - DJVU 파일의 차이점 비교"
head_description: "Java, J2EE, J2SE 애플리케이션에서 DJVU 파일을 비교하고 병합합니다. DJVU 파일, 이미지 및 문서 형식의 내용, 텍스트 및 스타일의 차이점 요약을 분석합니다."

############################# Header ############################
title: "Java의 DJVU 파일 비교"
description: "Java에서 두 개 이상의 DJVU 파일을 한 줄씩 비교합니다. 차이점 목록을 검색하고 비교된 파일을 단일 문서에 저장합니다."
...
steps:
    enable: true
    title_left: "Java에서 DJVU 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/java/)을 사용하면 Java 개발자가 몇 줄의 코드를 사용하여 애플리케이션 내에서 DJVU 파일을 쉽게 비교할 수 있습니다.
    ...
    code: |
        ...
        try (Comparer comparer = new Comparer("C:\\source.djvu")) {
            ...
            comparer.compare("C:\\result.djvu"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        try (Comparer comparer = new Comparer(new FileInputStream("C:\\source.djvu"))) {
            ...
            comparer.compare(new FileOutputStream("C:\\result.djvu")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "DJVU 파일 비교를 위한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DJVU 파일을 비교해 보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-djvu"
          title: "DJVU 파일 형식 정보"
          content: |
            “déjà vu”로 발음되는 DjVu는 스캔한 문서 및 책, 특히 텍스트, 그림, 이미지 및 사진의 조합을 포함하는 책을 위한 그래픽 파일 형식입니다. AT&T Labs에서 개발했습니다. 텍스트와 배경 이미지의 이미지 레이어 분리, 점진적 로딩, 산술 코딩, 흑백 이미지에 대한 손실 압축과 같은 여러 기술을 사용합니다. DJVU 파일은 압축되었지만 고품질의 컬러 이미지, 사진, 텍스트 및 그림을 포함할 수 있고 더 적은 공간에 저장할 수 있으므로 웹에서 전자책, 설명서, 신문, 고대 문서 등으로 사용됩니다.
