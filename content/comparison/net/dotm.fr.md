...
head_description: "Comparez et fusionnez plus de deux fichiers DOTM dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de document DOTM."

############################# Header ############################
title: "Comparez les fichiers DOTM en C# .NET"
description: "API de comparaison de documents .NET pour détecter les changements entre deux versions de fichiers DOTM et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers DOTM dans leurs applications en mettant en œuvre quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.dotm"))
        {
            ...
            comparer.Compare("result.dotm"); // Crée un fichier résultat avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.dotm")))
        {
            ...
            comparer.Compare(File.Create("result.dotm")); // Crée un fichier résultat avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de la comparaison de fichiers DOTM"
    content: |
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dotm"
          ...
          content: |
            Un fichier avec l'extension DOTM représente un fichier modèle créé avec Microsoft Word 2007 ou supérieur. Il est similaire au format de fichier DOCX populaire, sauf qu'il conserve les paramètres définis par l'utilisateur pour une réutilisation en cas de création de nouveaux documents. Ces documents sont plus souvent utilisés dans les bureaux où un fichier de modèle standard est généré avec des paramètres tels que les informations de page, les marges, la mise en page par défaut et les macros, et est utilisé pour créer de nouveaux documents à partir de celui-ci si nécessaire. Les fichiers DOTM, cependant, enregistrent des macros, qui sont une série de commandes sous la forme d'actions enregistrées pour l'achèvement automatique d'une tâche. Cela permet de gagner du temps lors de l'exécution d'actions répétées à la fin d'une tâche.
