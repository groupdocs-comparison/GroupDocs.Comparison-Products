...
############################# Head ############################
head_title: ".NET で 2 つの XLTM ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の XLTM ファイルを比較およびマージします。 XLTM ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での XLTM ファイルの比較"
description: ".NET ドキュメント比較 API。XLTM ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で XLTM ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の XLTM ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.xltm"))
        {
            ...
            comparer.Compare("result.xltm"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.xltm")))
        {
            ...
            comparer.Compare(File.Create("result.xltm")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "XLTM ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ XLTM ファイル間の違いを検出してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-xltm"
          title: "XLTM ファイル形式について"
          content: |
            XLTM ファイル拡張子は、Microsoft Excel によってマクロ有効テンプレート ファイルとして生成されたファイルを表します。 XLTM ファイルは、マクロを使用したテンプレート ファイルの作成をサポートしていない点を除けば、XLTX と構造が似ています。このようなテンプレート ファイルは、マクロとともにレイアウト、書式設定、その他の設定を生成および設定するために使用され、同様の XLSX ファイルの作成を容易にします。
