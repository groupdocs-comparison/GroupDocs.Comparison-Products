...
############################# Head ############################
head_title: "Comparez deux fichiers DOTX dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers DOTX dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents DOTX."

############################# Header ############################
title: "Comparez les fichiers DOTX en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers DOTX et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers DOTX en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers DOTX dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.dotx"))
        {
            ...
            comparer.Compare("result.dotx"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.dotx")))
        {
            ...
            comparer.Compare(File.Create("result.dotx")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers DOTX"
    content: |
        Détectez dès maintenant les différences entre les fichiers DOTX en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dotx"
          ...
          content: |
            Les fichiers avec l'extension DOTX sont des fichiers modèles créés par Microsoft Word pour avoir des paramètres préformatés pour la génération d'autres fichiers DOCX. Un fichier modèle est créé afin d'avoir des paramètres utilisateur spécifiques qui doivent être appliqués aux mouches ultérieures créées à partir de ceux-ci. Ces paramètres incluent les marges de page, les bordures, les en-têtes, les pieds de page et d'autres paramètres de page. De tels modèles sont utilisés dans les documents officiels tels que les papiers à en-tête des entreprises et les formulaires standardisés. Le format de fichier DOTX a été introduit avec la sortie de Microsoft Office 2007 pour remplacer le format de fichier binaire DOT, mais il est également pris en charge par les versions supérieures. Microsoft Word ouvre par défaut chaque nouveau document basé sur le fichier normal.dot. S'ils sont modifiés, tous les nouveaux fichiers créés entraîneront les mêmes paramètres que ceux du fichier modèle. Dans Microsoft Word 2007, le format de fichier DOT a été remplacé par le format de fichier DOTX basé sur Office OpenXML.
