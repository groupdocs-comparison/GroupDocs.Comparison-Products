...
############################# Head ############################
head_title: ".NET에서 두 XLSX 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 XLSX 파일을 비교하고 병합합니다. XLSX 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET에서 XLSX 파일 비교"
description: ".NET 문서 비교 API는 XLSX 파일의 두 버전 간 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 XLSX 파일을 쉽게 비교 및 ​​병합할 수 있습니다.
...
demos:
    enable: true
    title: "XLSX 파일 비교 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 XLSX 파일 간의 차이점을 감지하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-xlsx"
          title: "XLSX 파일 형식 정보"
          content: |
            XLSX는 Microsoft Office 2007 릴리스와 함께 Microsoft에서 도입한 잘 알려진 Microsoft Excel 문서 형식입니다. OOXML 표준 ECMA-376의 파트 2에 설명된 개방형 패키징 규칙에 따라 구성된 구조를 기반으로 하는 새 형식은 다음과 같습니다. 여러 XML 파일이 포함된 zip 패키지입니다. .xlsx 파일의 압축을 풀기만 하면 기본 구조와 파일을 검사할 수 있습니다.
