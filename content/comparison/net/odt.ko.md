...
############################# Head ############################
head_title: ".NET에서 두 개의 ODT 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 ODT 파일을 비교하고 병합합니다. ODT 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET의 ODT 파일 비교"
description: ".NET 문서 비교 API는 ODT 파일의 두 버전 간의 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    enable: true
    title_left: "C#에서 ODT 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 ODT 파일을 쉽게 비교하고 병합할 수 있습니다.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.odt"))
        {
            ...
            comparer.Compare("result.odt"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.odt")))
        {
            ...
            comparer.Compare(File.Create("result.odt")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "ODT 파일 비교에 대한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 ODT 파일 간의 차이점을 찾아보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-odt"
          title: "ODT 파일 형식 정보"
          content: |
            ODT 파일은 OpenDocument 텍스트 파일 형식을 기반으로 하는 워드 프로세싱 응용 프로그램으로 만든 문서 유형입니다. 이는 무료 OpenOffice Writer와 같은 워드 프로세서 응용 프로그램으로 생성되며 텍스트, 이미지, 개체 및 스타일과 같은 콘텐츠를 저장할 수 있습니다. ODT 파일은 Microsoft Word의 DOCX와 마찬가지로 Writer 워드 프로세서의 파일입니다. Google Docs 및 Google Drive에 포함된 Google의 웹 기반 워드 프로세서를 포함한 여러 애플리케이션은 편집을 위해 ODT 파일을 열 수 있습니다. Microsoft Word에서는 ODT 파일을 열고 DOC 및 DOCX와 같은 다른 형식으로 저장할 수도 있습니다.
