...
############################# Head ############################
head_title: "在 .NET 中比较两个 ODT 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 ODT 文件。检索 ODT 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 ODT 文件"
description: ".NET 文档比较 API 检测两个版本的 ODT 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 ODT 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 ODT 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.odt"))
        {
            ...
            comparer.Compare("result.odt"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.odt")))
        {
            ...
            comparer.Compare(File.Create("result.odt")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 ODT 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 ODT 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-odt"
          title: "关于 ODT 文件格式"
          content: |
            ODT 文件是使用基于 OpenDocument 文本文件格式的文字处理应用程序创建的文档类型。它们是使用文字处理器应用程序创建的，例如免费的 OpenOffice Writer，可以保存文本、图像、对象和样式等内容。 ODT 文件之于 Writer 字处理器就像 DOCX 之于 Microsoft Word。包括 Google Docs 和 Google Drive 中包含的基于网络的 Google 文字处理器在内的多个应用程序都可以打开 ODT 文件进行编辑。 Microsoft Word 还可以打开 ODT 文件并将其保存为其他格式，例如 DOC 和 DOCX。
