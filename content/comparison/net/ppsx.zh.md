...
############################# Head ############################
head_title: "在 .NET 中比较两个 PPSX 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 PPSX 文件。检索 PPSX 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 PPSX 文件"
description: ".NET 文档比较 API 检测两个版本的 PPSX 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 PPSX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 PPSX 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.ppsx"))
        {
            ...
            comparer.Compare("result.ppsx"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.ppsx")))
        {
            ...
            comparer.Compare(File.Create("result.ppsx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 PPSX 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 PPSX 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-ppsx"
          title: "关于 PPSX 文件格式"
          content: |
            PPSX、Power Point 幻灯片、文件是使用 Microsoft PowerPoint 2007 及更高版本创建的，用于幻灯片放映。它是对 Microsoft PowerPoint 97-2003 版本支持的 PPS 文件格式的更新。当 PPSX 文件与其他用户共享并打开时，它以 PowerPoint 显示形式启动，这与以可编辑模式打开的 PPTX 文件不同。幻灯片放映的顺序与原始演示文稿中的顺序相同。在幻灯片放映期间，所有幻灯片都伴随着图像、声音和其他嵌入媒体伴随着演示幻灯片到 PPSX。
