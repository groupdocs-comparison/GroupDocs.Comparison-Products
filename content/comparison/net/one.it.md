...
############################# Head ############################
head_title: "Confronta due file ONE in .NET | API di confronto dei documenti"
head_description: "Confronta e unisci più di due file ONE in applicazioni C# .NET. Recupera il riepilogo delle differenze nel contenuto, nel testo e nello stile di ONE file, immagini e formati di documenti."

############################# Header ############################
title: "Confronta i file ONE in C# .NET"
description: "API di confronto dei documenti .NET per rilevare le modifiche tra due versioni di file ONE ed esportare in un documento finale con un riepilogo dettagliato delle differenze tra i documenti confrontati."
...
steps:
    enable: true
    title_left: "Passaggi per confrontare i file ONE in C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) semplifica agli sviluppatori .NET il confronto e l'unione di più file ONE nelle loro applicazioni implementando pochi semplici passaggi.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.one"))
        {
            ...
            comparer.Compare("result.one"); // Crea il file dei risultati con il nome specificato
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.one")))
        {
            ...
            comparer.Compare(File.Create("result.one")); // Crea il file dei risultati con il nome specificato
        }
        ```
...
demos:
    enable: true
    title: "Demo live di confronto di ONE file"
    content: |
        Rileva subito le differenze tra i file ONE visitando il sito web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-one"
          title: "Informazioni sul formato file ONE"
          content: |
            Il file rappresentato dall'estensione .ONE viene creato dall'applicazione Microsoft OneNote. OneNote ti consente di raccogliere informazioni utilizzando l'applicazione come se stessi utilizzando il blocco per appunti per prendere appunti. I file di OneNote possono contenere diversi elementi che possono essere posizionati in posizioni non fisse nelle pagine del documento. Questi elementi possono contenere testo, calligrafia digitalizzata e oggetti copiati da altre applicazioni, tra cui immagini, disegni e clip multimediali (audio/video). Microsoft ora offre la versione online di OneNote come parte di Office365 in cui le note possono essere condivise con altri utenti di OneNote su Internet.
