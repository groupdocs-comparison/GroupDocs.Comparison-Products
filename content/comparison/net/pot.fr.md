...
############################# Head ############################
head_title: "Comparez deux fichiers POT dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers POT dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents POT."

############################# Header ############################
title: "Comparez les fichiers POT en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers POT et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers POT en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers POT dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.pot"))
        {
            ...
            comparer.Compare("result.pot"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.pot")))
        {
            ...
            comparer.Compare(File.Create("result.pot")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers POT"
    content: |
        Détectez dès maintenant les différences entre les fichiers POT en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-pot"
          title: "À propos du format de fichier POT"
          content: |
            Les fichiers avec l'extension .POT représentent les fichiers de modèles Microsoft PowerPoint créés par les versions PowerPoint 97-2003. Les fichiers créés avec ces versions de Microsoft PowerPoint sont au format binaire par rapport à ceux créés dans les formats de fichiers Office OpenXML à l'aide des versions supérieures de PowerPoint. Les fichiers ainsi générés peuvent être utilisés pour créer des présentations ayant la même mise en page et d'autres paramètres devant être appliqués aux nouveaux fichiers. Ces paramètres peuvent inclure des styles, des arrière-plans, une palette de couleurs, des polices et des valeurs par défaut. Ces fichiers sont générés afin de créer des fichiers modèles prêts à l'emploi pour un usage officiel.
