...
############################# Head ############################
head_title: "Comparez deux fichiers ASM dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers ASM dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents ASM."

############################# Header ############################
title: "Comparez les fichiers ASM en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers ASM et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers ASM en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers ASM dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.asm"))
        {
            ...
            comparer.Compare("result.asm"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.asm")))
        {
            ...
            comparer.Compare(File.Create("result.asm")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers ASM"
    content: |
        Détectez dès maintenant les différences entre les fichiers ASM en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-asm"
          title: "À propos du format de fichier ASM"
          content: |
            {{asm}}
