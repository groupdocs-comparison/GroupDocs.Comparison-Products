...
############################# Head ############################
head_title: "Сравните два файла HTM в .NET | API-интерфейсы сравнения документов"
head_description: "Сравнивайте и объединяйте более двух файлов HTM в приложениях C# .NET. Получите сводную информацию о различиях в содержании, тексте и стиле файлов, изображений и форматов документов HTM."

############################# Header ############################
title: "Сравнить файлы HTM в C# .NET"
description: "API сравнения документов .NET для обнаружения изменений между двумя версиями файлов HTM и экспорта в окончательный документ с подробным описанием различий между сравниваемыми документами."
...
steps:
    enable: true
    title_left: "Действия по сравнению файлов HTM в C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) позволяет разработчикам .NET сравнивать и объединять несколько файлов HTM в своих приложениях, выполнив несколько простых шагов.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.htm"))
        {
            ...
            comparer.Compare("result.htm"); // Создать файл результатов с указанным именем
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.htm")))
        {
            ...
            comparer.Compare(File.Create("result.htm")); // Создать файл результатов с указанным именем
        }
        ```
...
demos:
    enable: true
    title: "Живые демонстрации сравнения файлов HTM"
    content: |
        Обнаружьте различия между файлами HTM прямо сейчас, посетив веб-сайт [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-htm"
          ...
          content: |
            Файлы с расширением HTM представляют собой язык гипертекстовой разметки для создания веб-страниц для отображения в таких веб-браузерах, как Google Chrome, Internet Explorer, Firefox и ряде других. Он определяет разметку для создания статических страниц, которые будут опубликованы во Всемирной паутине (WWW) для доступа других. Эти разметки сообщают браузерам, как отображать содержимое веб-страницы. Такие страницы могут содержать простой текст, изображения, гиперссылки на другие страницы, видео и другую медиаинформацию. Когда веб-страница публикуется, вы можете просмотреть код разметки, стоящий за ней, просмотрев исходный код ее страницы. Современные браузеры позволяют проверять каждый раздел веб-страницы, где детально разработано каждое подразделение или элемент разметки в исходном коде HTM.
