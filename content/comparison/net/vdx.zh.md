...
############################# Head ############################
head_title: "在 .NET 中比较两个 VDX 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 VDX 文件。检索 VDX 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 VDX 文件"
description: ".NET 文档比较 API 检测两个版本的 VDX 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 VDX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 VDX 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.vdx"))
        {
            ...
            comparer.Compare("result.vdx"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.vdx")))
        {
            ...
            comparer.Compare(File.Create("result.vdx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 VDX 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 VDX 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vdx"
          title: "关于 VDX 文件格式"
          content: |
            在 Microsoft Visio 中创建但以 XML 格式保存的任何绘图或图表都具有 .VDX 扩展名。 Visio 绘图 XML 文件是在 Microsoft 开发的 Visio 软件中创建的。 Microsoft Visio 能够生成可用于演示文稿和文档的可视化文档。 Visio 绘图 XML 文件包含视觉对象和视觉元素的元数据详细信息。也可以将文本添加到这些视觉元素 Vision 绘图 XML 文件中。这些 Visio 绘图 XML 文件集成了基于 XML 的格式设置标准和图像数据编码规范，允许其内容由 Microsoft Visio 软件以 VDX 文件格式呈现和存储。
