...
############################# Head ############################
head_title: ".NETの2つのHTMファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのHTMファイル。HTMファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでHTMファイルを比較する"
description: ".2つのバージョンのHTMファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でHTMファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のHTMファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.htm"))
        {
            ...
            comparer.Compare("result.htm"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.htm")))
        {
            ...
            comparer.Compare(File.Create("result.htm")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    enable: true
    title: "HTMファイルの比較のライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、HTMファイル間の違いを今すぐ検出してください。  
        ライブデモには次の利点があります
