...
############################# Head ############################
head_title: "Confronta due file PY in .NET | API di confronto dei documenti"
head_description: "Confronta e unisci più di due file PY in applicazioni C# .NET. Recupera il riepilogo delle differenze nel contenuto, nel testo e nello stile di PY file, immagini e formati di documenti."

############################# Header ############################
title: "Confronta i file PY in C# .NET"
description: "API di confronto dei documenti .NET per rilevare le modifiche tra due versioni di file PY ed esportare in un documento finale con un riepilogo dettagliato delle differenze tra i documenti confrontati."
...
steps:
    enable: true
    title_left: "Passaggi per confrontare i file PY in C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) semplifica agli sviluppatori .NET il confronto e l'unione di più file PY nelle loro applicazioni implementando pochi semplici passaggi.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.py"))
        {
            ...
            comparer.Compare("result.py"); // Crea il file dei risultati con il nome specificato
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.py")))
        {
            ...
            comparer.Compare(File.Create("result.py")); // Crea il file dei risultati con il nome specificato
        }
        ```
...
demos:
    enable: true
    title: "Demo live di confronto di PY file"
    content: |
        Rileva subito le differenze tra i file PY visitando il sito web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        # format loop
        - icon: "far fa-file-py"
          title: "Informazioni sul formato file PY"
          content: |
            I file con estensione .py contengono il codice sorgente Python. È un linguaggio molto popolare che può essere utilizzato per lo sviluppo web, lo sviluppo di software, la matematica e lo scripting di sistema. Python è un linguaggio multipiattaforma che può funzionare su diverse piattaforme come Windows, MAC, Linux, Raspberry Pi, ecc. Python fornisce una sintassi semplice simile alla lingua inglese ed è più facile da leggere. La sintassi di Python consente di scrivere programmi con meno righe di codice. Python viene eseguito su un sistema di interpreti in modo che il codice possa essere eseguito non appena viene scritto, il che lo rende molto utile per la prototipazione.
