...
############################# Head ############################
head_title: "在 .NET 中比较两个 CPP 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 CPP 文件。检索 CPP 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 CPP 文件"
description: ".NET 文档比较 API 检测两个版本的 CPP 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 CPP 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 CPP 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cpp"))
        {
            ...
            comparer.Compare("result.cpp"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cpp")))
        {
            ...
            comparer.Compare(File.Create("result.cpp")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 CPP 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 CPP 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cpp"
          title: "关于 CPP 文件格式"
          content: |
            具有 CPP 文件扩展名的文件是用 C++ 编程语言编写的应用程序的源代码文件。一个 C++ 项目可能包含多个 CPP 文件作为应用程序源代码。这样的项目由不同的文件类型组成，其中 CPP 文件被称为实现文件，因为它们包含头文件 (.h) 中声明的方法的所有定义。当作为一个整体编译时，C++ 项目作为一个整体产生一个可执行应用程序。
