...
############################# Head ############################
head_title: "比較 .NET 中的兩個 CPP 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 CPP 文件。檢索 CPP 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 CPP 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 CPP 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 CPP 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 CPP 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cpp"))
        {
            ...
            comparer.Compare("result.cpp"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cpp")))
        {
            ...
            comparer.Compare(File.Create("result.cpp")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 CPP 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 CPP 文件之間的差異。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cpp"
          title: "關於 CPP 文件格式"
          content: |
            具有 CPP 文件擴展名的文件是用 C++ 編程語言編寫的應用程序的源代碼文件。單個 C++ 項目可能包含多個 CPP 文件作為應用程序源代碼。這樣的項目由不同的文件類型組成，其中 CPP 文件稱為實現文件，因為它們包含頭文件 (.h) 中聲明的方法的所有定義。當作為一個整體進行編譯時，C++ 項目作為一個整體會產生一個可執行的應用程序。
