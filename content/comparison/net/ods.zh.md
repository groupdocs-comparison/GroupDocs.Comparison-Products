...
############################# Head ############################
head_title: "比較 .NET 中的兩個 ODS 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 ODS 文件。檢索 ODS 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 ODS 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 ODS 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 ODS 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 ODS 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.ods"))
        {
            ...
            comparer.Compare("result.ods"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.ods")))
        {
            ...
            comparer.Compare(File.Create("result.ods")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 ODS 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 ODS 文件之間的差異。
        現場演示有以下好處
