...
############################# Head ############################
head_title: "在 .NET 中比较两个 PPTX 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 PPTX 文件。检索 PPTX 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 PPTX 文件"
description: ".NET 文档比较 API 检测两个版本的 PPTX 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 PPTX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 PPTX 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.pptx"))
        {
            ...
            comparer.Compare("result.pptx"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.pptx")))
        {
            ...
            comparer.Compare(File.Create("result.pptx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 PPTX 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 PPTX 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-pptx"
          title: "关于 PPTX 文件格式"
          content: |
            带有 PPTX 扩展名的文件是使用流行的 Microsoft PowerPoint 应用程序创建的演示文稿文件。与以前版本的二进制演示文稿文件格式 PPT 不同，PPTX 格式基于 Microsoft PowerPoint 开放 XML 演示文稿文件格式。演示文稿文件是幻灯片的集合，其中每张幻灯片都可以包含文本、图像、格式、动画和其他媒体。这些幻灯片以带有自定义演示设置的幻灯片形式呈现给观众。
