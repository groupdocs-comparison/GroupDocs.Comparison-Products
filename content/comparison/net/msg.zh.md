...
############################# Head ############################
head_title: "比較 .NET 中的兩個 MSG 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 MSG 文件。檢索 MSG 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 MSG 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 MSG 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 MSG 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 MSG 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.msg"))
        {
            ...
            comparer.Compare("result.msg"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.msg")))
        {
            ...
            comparer.Compare(File.Create("result.msg")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 MSG 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 MSG 文件之間的差異。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-msg"
          title: "關於 MSG 文件格式"
          content: |
            MSG 是 Microsoft Outlook 和 Exchange 用於存儲電子郵件、聯繫人、約會或其他任務的文件格式。此類消息可以包含一個或多個電子郵件字段，其中包含發件人、收件人、主題、日期和消息正文，或聯繫信息、約會細節和一項或多項任務規範。構成Message對象的屬性，包括也是MSG文件的一部分。 MSG 文件具有純 ASCII 文本形式的標題、主消息正文和超鏈接。 MSG 文件還適用於需要 Microsoft 消息應用程序編程接口 (MAPI) 的程序。
