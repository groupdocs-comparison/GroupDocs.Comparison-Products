...
############################# Head ############################
head_title: "在 .NET 中比较两个 MSG 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 MSG 文件。检索 MSG 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 MSG 文件"
description: ".NET 文档比较 API 检测两个版本的 MSG 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 MSG 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 MSG 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.msg"))
        {
            ...
            comparer.Compare("result.msg"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.msg")))
        {
            ...
            comparer.Compare(File.Create("result.msg")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 MSG 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 MSG 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-msg"
          title: "关于 MSG 文件格式"
          content: |
            MSG 是 Microsoft Outlook 和 Exchange 用来存储电子邮件、联系人、约会或其他任务的文件格式。此类消息可能包含一个或多个电子邮件字段，包括发件人、收件人、主题、日期和消息正文，或联系信息、约会详情和一项或多项任务规范。构成 Message 对象的属性，包括也是 MSG 文件的一部分。 MSG 文件以纯 ASCII 文本形式包含标头、主要消息正文和超链接。 MSG 文件也适用于需要 Microsoft 消息应用程序编程接口 (MAPI) 的程序。
