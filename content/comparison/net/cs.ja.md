...
############################# Head ############################
head_title: ".NET で 2 つの CS ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の CS ファイルを比較およびマージします。 CS ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での CS ファイルの比較"
description: ".NET ドキュメント比較 API。CS ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で CS ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の CS ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cs"))
        {
            ...
            comparer.Compare("result.cs"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cs")))
        {
            ...
            comparer.Compare(File.Create("result.cs")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "CS ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ CS ファイル間の違いを検出してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cs"
          title: "CS ファイル形式について"
          content: |
            拡張子が .CS のファイルは、C# プログラミング言語のソース コード ファイルです。 .NET Framework で使用するために Microsoft によって導入されたこのファイル形式は、EXE または DLL の形式で最終出力ファイルを生成するためにコンパイルされるコードを記述するための低レベル プログラミング言語を提供します。これらは、Microsoft Visual Studio を使用して作成およびコンパイルできます。無料の IDE である Microsoft Visual Studio Express を使用して、このようなファイルを作成および更新することもできます。 CS ファイルは、単純なデスクトップ アプリケーションからより複雑なプログラムに至るまで、アプリケーション開発に使用されます。 C# 言語で作成された単純な Visual Studio プロジェクト ソリューションは、1 つ以上のそのようなファイルで構成されます。コンパイルに含めるようマークされたファイルは、プロジェクトの一部である CSPROJ ファイルにリストされ、マークされたファイルを使用するようにコンパイラに指示します。
