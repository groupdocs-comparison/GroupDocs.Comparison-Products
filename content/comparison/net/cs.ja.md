...
############################# Head ############################
head_title: ".NETの2つのCSファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのCSファイル。CSファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでCSファイルを比較する"
description: ".2つのバージョンのCSファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でCSファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のCSファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cs"))
        {
            ...
            comparer.Compare("result.cs"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cs")))
        {
            ...
            comparer.Compare(File.Create("result.cs")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cs"
          title: "CSファイル形式について"
          content: |
            拡張子が.CSのファイルは、C＃プログラミング言語のソースコードファイルです。 .NET Frameworkで使用するためにMicrosoftによって導入されたファイル形式は、EXEまたはDLLの形式で最終出力ファイルを生成するためにコンパイルされるコードを記述するための低レベルのプログラミング言語を提供します。これらは、MicrosoftVisualStudioを使用して作成およびコンパイルできます。 Microsoft Visual Studio Expressを使用して、無料のIDEであるこのようなファイルを作成および更新することもできます。 CSファイルは、単純なデスクトップアプリケーションからより複雑なプログラムまで、さまざまなアプリケーション開発に使用されます。 C＃言語で作成された単純なVisual Studioプロジェクトソリューションは、1つ以上のそのようなファイルで構成できます。コンパイルに含めるようにマークされたファイルは、プロジェクトの一部であり、マークされたファイルを使用するようにコンパイラーに指示するCSPROJファイルにリストされます。
