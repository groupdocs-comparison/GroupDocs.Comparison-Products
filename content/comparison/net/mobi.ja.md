...
############################# Head ############################
head_title: ".NETの2つのMOBIファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのMOBIファイル。MOBIファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでMOBIファイルを比較する"
description: ".2つのバージョンのMOBIファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でMOBIファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のMOBIファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.mobi"))
        {
            ...
            comparer.Compare("result.mobi"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.mobi")))
        {
            ...
            comparer.Compare(File.Create("result.mobi")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    enable: true
    title: "MOBIファイルの比較のライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、MOBIファイル間の違いを今すぐ検出してください。  
        ライブデモには次の利点があります
