...
############################# Head ############################
head_title: "Compare dois arquivos DOC em .NET | APIs de comparação de documentos"
head_description: "Compare e mescle mais de dois arquivos DOC em aplicativos C# .NET. Recupere resumos de diferenças em conteúdo, texto e estilo de DOC arquivos, imagens e formatos de documentos."

############################# Header ############################
title: "Compare arquivos DOC em C# .NET"
description: "API de comparação de documentos .NET para detectar as alterações entre duas versões de arquivos DOC e exportar para um documento final com um resumo detalhado das diferenças entre os documentos comparados."
...
steps:
    enable: true
    title_left: "Etapas para comparar arquivos DOC em C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) torna mais fácil para os desenvolvedores .NET comparar e mesclar vários arquivos DOC em seus aplicativos, implementando algumas etapas fáceis.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.doc"))
        {
            ...
            comparer.Compare("result.doc"); // Crie um arquivo de resultado com o nome especificado
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.doc")))
        {
            ...
            comparer.Compare(File.Create("result.doc")); // Crie um arquivo de resultado com o nome especificado
        }
        ```
...
demos:
    enable: true
    title: "Demonstrações ao vivo de comparação de arquivos DOC"
    content: |
        Detecte diferenças entre arquivos DOC agora mesmo visitando o site [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-doc"
          ...
          content: |
            Arquivos com extensão .doc representam documentos gerados pelo Microsoft Word ou outros documentos de processamento de texto em formato de arquivo binário. A extensão foi inicialmente usada para documentação de texto simples em vários sistemas operacionais diferentes. Ele pode conter vários tipos diferentes de dados, como imagens, texto formatado e também texto simples, gráficos, tabelas, objetos incorporados, links, páginas, formatação de página, configurações de impressão e muitos outros. O formato se popularizou para todo tipo de documentação pela variedade de opções que oferece aos usuários para a redação de manuais, propostas, especificações, currículos, artigos ou quaisquer documentos semelhantes. A versão atualizada do DOC é o DOCX, que é baseado no Office OpenXML cujas especificações estão disponíveis abertamente.
