...
############################# Head ############################
head_title: ".NET에서 두 VDX 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 VDX 파일을 비교하고 병합합니다. VDX 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET에서 VDX 파일 비교"
description: ".NET 문서 비교 API는 VDX 파일의 두 버전 간 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 VDX 파일을 쉽게 비교 및 ​​병합할 수 있습니다.
...
demos:
    enable: true
    title: "VDX 파일 비교 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 VDX 파일 간의 차이점을 감지하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-vdx"
          title: "VDX 파일 형식 정보"
          content: |
            Microsoft Visio에서 생성되었지만 XML 형식으로 저장된 모든 그림 또는 차트는 .VDX 확장자를 갖습니다. Visio 드로잉 XML 파일은 Microsoft에서 개발한 Visio 소프트웨어에서 생성됩니다. Microsoft Visio에는 프레젠테이션 및 문서에서 사용할 수 있는 시각적 문서를 생성하는 기능이 있습니다. Visio 드로잉 XML 파일에는 시각적 개체와 시각적 요소의 메타데이터 세부 정보가 포함되어 있습니다. 이러한 시각적 요소에 텍스트를 추가할 수도 있습니다. 비전 도면 XML 파일. 이러한 Visio 드로잉 XML 파일은 Microsoft Visio 소프트웨어에서 해당 콘텐츠를 VDX 파일 형식으로 렌더링하고 저장할 수 있도록 하는 XML 기반 서식 표준 및 이미지 데이터 인코딩 사양과 통합됩니다.
