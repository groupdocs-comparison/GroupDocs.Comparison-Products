...
############################# Head ############################
head_title: "在 .NET 中比较两个 PPT 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 PPT 文件。检索 PPT 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 PPT 文件"
description: ".NET 文档比较 API 检测两个版本的 PPT 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 PPT 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 PPT 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.ppt"))
        {
            ...
            comparer.Compare("result.ppt"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.ppt")))
        {
            ...
            comparer.Compare(File.Create("result.ppt")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 PPT 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 PPT 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-ppt"
          title: "关于 PPT 文件格式"
          content: |
            带有 PPT 扩展名的文件代表 PowerPoint 文件，它由一组幻灯片组成，用于显示为 SlideShow。它指定 Microsoft PowerPoint 97-2003 使用的二进制文件格式。 PPT 文件可以包含多种不同类型的信息，例如文本、项目符号、图像、多媒体和其他嵌入的 OLE 对象。微软从 2007 年开始为 PowerPoint 提出了新的文件格式，称为 PPTX，它基于 Office OpenXML，不同于这种二进制文件格式。其他几个应用程序，如 OpenOffice Impress 和 Apple Keynote 也可以创建 PPT 文件。
