...
############################# Head ############################
head_title: ".NET で 2 つの JS ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の JS ファイルを比較およびマージします。 JS ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での JS ファイルの比較"
description: ".NET ドキュメント比較 API。JS ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で JS ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の JS ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.js"))
        {
            ...
            comparer.Compare("result.js"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.js")))
        {
            ...
            comparer.Compare(File.Create("result.js")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "JS ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ JS ファイル間の違いを検出してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-js"
          title: "JS ファイル形式について"
          content: |
            JS (JavaScript) は、Web ページ上で実行するための JavaScript コードを含むファイルです。 JavaScript ファイルは .js 拡張子で保存されます。 HTML ドキュメント内には、タグを使用して JavaScript コードを埋め込むか、JS ファイルを含めることができます。 CSS ファイルと同様に、JS ファイルはコードを再利用できるように複数の HTML ドキュメントに含めることができます。 JavaScript を使用して HTML DOM を操作できます。
