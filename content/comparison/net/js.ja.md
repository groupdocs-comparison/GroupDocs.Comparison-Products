...
############################# Head ############################
head_title: ".NETの2つのJSファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのJSファイル。JSファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでJSファイルを比較する"
description: ".2つのバージョンのJSファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でJSファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のJSファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.js"))
        {
            ...
            comparer.Compare("result.js"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.js")))
        {
            ...
            comparer.Compare(File.Create("result.js")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    enable: true
    title: "JSファイルの比較のライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、JSファイル間の違いを今すぐ検出してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-js"
          title: "JSファイル形式について"
          content: |
            JS（JavaScript）は、Webページで実行するためのJavaScriptコードを含むファイルです。 JavaScriptファイルは.js拡張子で保存されます。 HTMLドキュメント内には、タグを使用してJavaScriptコードを埋め込むか、JSファイルを含めることができます。 CSSファイルと同様に、JSファイルはコードの再利用のために複数のHTMLドキュメントに含めることができます。 JavaScriptを使用してHTMLDOMを操作できます。
