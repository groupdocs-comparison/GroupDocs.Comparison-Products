...
############################# Head ############################
head_title: ".NETの2つのCPPファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのCPPファイル。CPPファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでCPPファイルを比較する"
description: ".2つのバージョンのCPPファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でCPPファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のCPPファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cpp"))
        {
            ...
            comparer.Compare("result.cpp"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cpp")))
        {
            ...
            comparer.Compare(File.Create("result.cpp")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    enable: true
    title: "CPPファイルの比較のライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、CPPファイル間の違いを今すぐ検出してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cpp"
          title: "CPPファイル形式について"
          content: |
            CPPファイル拡張子の付いたファイルは、C++プログラミング言語で記述されたアプリケーションのソースコードファイルです。単一のC++プロジェクトには、アプリケーションのソースコードとして複数のCPPファイルが含まれる場合があります。このようなプロジェクトはさまざまなファイルタイプで構成されており、CPPファイルにはヘッダー（.h）ファイルで宣言されたメソッドのすべての定義が含まれているため、実装ファイルと呼ばれます。全体としてのC++プロジェクトは、全体としてコンパイルされたときに実行可能アプリケーションになります。
