...
############################# Head ############################
head_title: ".NET에서 두 개의 DIFF 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 DIFF 파일을 비교하고 병합합니다. DIFF 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET의 DIFF 파일 비교"
description: ".NET 문서 비교 API는 DIFF 파일의 두 버전 간의 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    enable: true
    title_left: "C#에서 DIFF 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 DIFF 파일을 쉽게 비교하고 병합할 수 있습니다.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.diff"))
        {
            ...
            comparer.Compare("result.diff"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.diff")))
        {
            ...
            comparer.Compare(File.Create("result.diff")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "DIFF 파일 비교에 대한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DIFF 파일 간의 차이점을 찾아보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-diff"
          title: "DIFF 파일 형식 정보"
          content: |
            DIFF 파일은 하나 이상의 텍스트 또는 소스 파일 간의 차이점을 보여주는 단일 파일입니다. 개발자가 파일 간의 차이점을 파악하고 패치를 적용할 수 있는 제어 관리 도구인 Mercurial에서 만들었습니다. 개발자는 DIFF 파일을 통해 변경 사항을 다른 개발자와 공유하므로 변경 사항을 업데이트하여 최신 버전의 텍스트 파일을 생성할 수 있습니다. DIFF 파일은 Mercurial에서 열 수 있습니다.
