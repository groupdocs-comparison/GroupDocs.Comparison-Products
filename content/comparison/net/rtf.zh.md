...
############################# Head ############################
head_title: "在 .NET 中比较两个 RTF 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 RTF 文件。检索 RTF 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 RTF 文件"
description: ".NET 文档比较 API 检测两个版本的 RTF 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 RTF 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 RTF 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.rtf"))
        {
            ...
            comparer.Compare("result.rtf"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.rtf")))
        {
            ...
            comparer.Compare(File.Create("result.rtf")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 RTF 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 RTF 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-rtf"
          title: "关于 RTF 文件格式"
          content: |
            富文本格式 (RTF) 由 Microsoft 引入并记录在案，代表了一种对格式化文本和图形进行编码以供在应用程序中使用的方法。该格式有助于与其他 Microsoft 产品进行跨平台文档交换，从而达到互操作性的目的。此功能使其成为文字处理软件之间数据传输的标准，因此内容可以从一个操作系统传输到另一个操作系统而不会丢失文档格式。文件格式规范由微软提供公开下载，可以从开发者的角度进行参考。
