...
############################# Head ############################
head_title: "Comparez deux fichiers PY dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers PY dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents PY."

############################# Header ############################
title: "Comparez les fichiers PY en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers PY et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers PY en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers PY dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.py"))
        {
            ...
            comparer.Compare("result.py"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.py")))
        {
            ...
            comparer.Compare(File.Create("result.py")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers PY"
    content: |
        Détectez dès maintenant les différences entre les fichiers PY en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        # format loop
        - icon: "far fa-file-py"
          title: "À propos du format de fichier PY"
          content: |
            Les fichiers avec l'extension .py contiennent le code source de Python. C'est un langage très populaire qui peut être utilisé pour le développement Web, le développement de logiciels, les mathématiques et les scripts système. Python est un langage multiplateforme qui peut fonctionner sur différentes plates-formes comme Windows, MAC, Linux, Raspberry Pi, etc. Python fournit une syntaxe simple, similaire à la langue anglaise et plus facile à lire. La syntaxe Python permet d'écrire des programmes avec moins de lignes de code. Python fonctionne sur un système interpréteur afin que le code puisse être exécuté dès son écriture, ce qui le rend très adapté au prototypage.
