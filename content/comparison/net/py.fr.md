...
head_description: "Comparez et fusionnez plus de deux fichiers PY dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de document PY."

############################# Header ############################
title: "Comparez les fichiers PY en C# .NET"
description: "API de comparaison de documents .NET pour détecter les changements entre deux versions de fichiers PY et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers PY dans leurs applications en mettant en œuvre quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.py"))
        {
            ...
            comparer.Compare("result.py"); // Crée un fichier résultat avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.py")))
        {
            ...
            comparer.Compare(File.Create("result.py")); // Crée un fichier résultat avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de la comparaison de fichiers PY"
    content: |
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-py"
          ...
          content: |
            Les fichiers avec l'extension .py contiennent le code source python. C'est un langage très populaire qui peut être utilisé pour le développement Web, le développement de logiciels, les mathématiques et les scripts système. Python est un langage multiplateforme qui peut fonctionner sur différentes plateformes telles que Windows, MAC, Linux, Raspberry Pi, etc. Python fournit une syntaxe simple similaire à la langue anglaise et plus facile à lire. La syntaxe Python permet d'écrire des programmes avec moins de lignes de code. Python s'exécute sur un système d'interprétation afin que le code puisse être exécuté dès qu'il est écrit, ce qui le rend très bon pour le prototypage.
