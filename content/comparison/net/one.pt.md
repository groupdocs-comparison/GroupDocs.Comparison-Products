...
############################# Head ############################
head_title: "Compare dois arquivos ONE em .NET | APIs de comparação de documentos"
head_description: "Compare e mescle mais de dois arquivos ONE em aplicativos C# .NET. Recupere o resumo das diferenças no conteúdo, texto e estilo de ONE arquivos, imagens e formatos de documento."

############################# Header ############################
title: "Comparar arquivos ONE em C# .NET"
description: "API de comparação de documentos .NET para detectar as alterações entre duas versões de arquivos ONE e exportar para um documento final com um resumo detalhado das diferenças entre os documentos comparados."
...
steps:
    enable: true
    title_left: "Etapas para comparar arquivos ONE em C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) torna mais fácil para os desenvolvedores .NET comparar e mesclar vários arquivos ONE em seus aplicativos implementando algumas etapas fáceis.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.one"))
        {
            ...
            comparer.Compare("result.one"); // Cria arquivo de resultado com o nome especificado
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.one")))
        {
            ...
            comparer.Compare(File.Create("result.one")); // Cria arquivo de resultado com o nome especificado
        }
        ```
...
demos:
    enable: true
    title: "Demonstrações ao vivo de comparação de arquivos ONE"
    content: |
        Detecte diferenças entre arquivos ONE agora mesmo visitando o site [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-one"
          title: "Sobre o formato de arquivo ONE"
          content: |
            O arquivo representado pela extensão .ONE é criado pelo aplicativo Microsoft OneNote. O OneNote permite que você colete informações usando o aplicativo como se estivesse usando o rascunho para fazer anotações. Os arquivos do OneNote podem conter diferentes elementos que podem ser colocados em locais não fixos nas páginas do documento. Esses elementos podem conter texto, manuscrito digitalizado e objetos copiados de outros aplicativos, incluindo imagens, desenhos e clipes de multimídia (áudio/vídeo). A Microsoft agora oferece uma versão online do OneNote como parte do Office365, onde as anotações podem ser compartilhadas com outros usuários do OneNote pela Internet.
