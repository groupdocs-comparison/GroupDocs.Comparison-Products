...
############################# Head ############################
head_title: "在 .NET 中比较两个 HTM 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 HTM 文件。检索 HTM 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 HTM 文件"
description: ".NET 文档比较 API 检测两个版本的 HTM 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 HTM 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 HTM 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.htm"))
        {
            ...
            comparer.Compare("result.htm"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.htm")))
        {
            ...
            comparer.Compare(File.Create("result.htm")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 HTM 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 HTM 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-htm"
          title: "关于 HTM 文件格式"
          content: |
            带有 HTM 扩展名的文件代表超文本标记语言，用于创建网页以在网络浏览器（如 Google Chrome、Internet Explorer、Firefox 和许多其他浏览器）中显示。它定义了用于创建要在万维网 (WWW) 上发布以供其他人访问的静态页面的标记。这些标记告诉浏览器如何显示网页的内容。此类页面可以包含纯文本、图像、指向其他页面的超链接、视频和其他媒体信息。当一个网页发布时，你可以通过查看它的页面源代码来了解它背后的标记代码。现代浏览器允许检查网页的每个部分，其中详细说明了 HTM 源中的每个子部分或标记元素。
