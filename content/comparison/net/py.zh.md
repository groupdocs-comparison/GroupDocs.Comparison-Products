...
############################# Head ############################
head_title: "比較 .NET 中的兩個 PY 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 PY 文件。檢索 PY 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 PY 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 PY 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 PY 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 PY 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.py"))
        {
            ...
            comparer.Compare("result.py"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.py")))
        {
            ...
            comparer.Compare(File.Create("result.py")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    ...
    title: "比較 PY 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 PY 文件之間的差異。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-py"
          title: "關於 PY 文件格式"
          content: |
            擴展名為 .py 的文件包含 python 源代碼。它是一種非常流行的語言，可用於 Web 開發、軟件開發、數學和系統腳本編寫。 Python是一種跨平台語言，可以在Windows、MAC、Linux、Raspberry Pi等不同平台上運行。Python提供了類似於英語的簡單語法，更易於閱讀。 Python 語法允許用更少的代碼行編寫程序。 Python 在解釋器系統上運行，因此代碼一寫就可以執行，這使得它非常適合原型設計。
