...
############################# Head ############################
head_title: "在 .NET 中比较两个 PY 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 PY 文件。检索 PY 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 PY 文件"
description: ".NET 文档比较 API 检测两个版本的 PY 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 PY 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 PY 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.py"))
        {
            ...
            comparer.Compare("result.py"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.py")))
        {
            ...
            comparer.Compare(File.Create("result.py")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    ...
    title: "比较 PY 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 PY 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-py"
          title: "关于 PY 文件格式"
          content: |
            扩展名为 .py 的文件包含 Python 源代码。它是一种非常流行的语言，可用于 Web 开发、软件开发、数学和系统脚本。 Python 是一种跨平台语言，可以在 Windows、MAC、Linux、Raspberry Pi 等不同平台上工作。Python 提供类似于英语的简单语法，更易于阅读。 Python 的语法允许用更少的代码行编写程序。 Python 在解释器系统上运行，因此代码可以在编写后立即执行，这使得它非常适合原型设计。
