...
############################# Head ############################
head_title: ".NET에서 두 개의 MSG 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 MSG 파일을 비교하고 병합합니다. MSG 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET의 MSG 파일 비교"
description: ".NET 문서 비교 API는 MSG 파일의 두 버전 간의 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    enable: true
    title_left: "C#에서 MSG 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 MSG 파일을 쉽게 비교하고 병합할 수 있습니다.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.msg"))
        {
            ...
            comparer.Compare("result.msg"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.msg")))
        {
            ...
            comparer.Compare(File.Create("result.msg")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "MSG 파일 비교에 대한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 MSG 파일 간의 차이점을 찾아보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-msg"
          title: "MSG 파일 형식 정보"
          content: |
            MSG는 Microsoft Outlook 및 Exchange에서 이메일 메시지, 연락처, 약속 또는 기타 작업을 저장하는 데 사용되는 파일 형식입니다. 이러한 메시지에는 보낸 사람, 받는 사람, 제목, 날짜, 메시지 본문 또는 연락처 정보, 약속 세부 사항 및 하나 이상의 작업 사양이 포함된 하나 이상의 이메일 필드가 포함될 수 있습니다. 메시지 개체를 구성하는 속성도 MSG 파일의 일부입니다. MSG 파일에는 일반 ASCII 텍스트로 헤더, 기본 메시지 본문 및 하이퍼링크가 있습니다. MSG 파일은 Microsoft의 MAPI(메시징 응용 프로그램 프로그래밍 인터페이스)가 필요한 프로그램에도 적합합니다.
