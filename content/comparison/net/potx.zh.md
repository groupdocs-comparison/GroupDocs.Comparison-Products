...
############################# Head ############################
head_title: "在 .NET 中比较两个 POTX 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 POTX 文件。检索 POTX 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 POTX 文件"
description: ".NET 文档比较 API 检测两个版本的 POTX 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 POTX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 POTX 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.potx"))
        {
            ...
            comparer.Compare("result.potx"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.potx")))
        {
            ...
            comparer.Compare(File.Create("result.potx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 POTX 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 POTX 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-potx"
          title: "关于 POTX 文件格式"
          content: |
            带有 .POTX 扩展名的文件代表使用 Microsoft PowerPoint 2007 及更高版本创建的 Microsoft PowerPoint 模板演示文稿。创建此格式是为了替换基于二进制文件格式并受 PowerPoint 97-2003 支持的 POT 文件格式。生成的文件可用于创建具有相同布局的演示文稿和应用于新文件所需的其他设置。这些设置可以包括样式、背景、调色板、字体和默认设置。生成此类文件是为了创建供官方使用的即用型模板文件。
