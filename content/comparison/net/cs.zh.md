...
############################# Head ############################
head_title: "比較 .NET 中的兩個 CS 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 CS 文件。檢索 CS 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 CS 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 CS 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 CS 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 CS 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.cs"))
        {
            ...
            comparer.Compare("result.cs"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.cs")))
        {
            ...
            comparer.Compare(File.Create("result.cs")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    ...
    title: "比較 CS 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 CS 文件之間的差異。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-cs"
          title: "關於 CS 文件格式"
          content: |
            擴展名為 .CS 的文件是 C# 編程語言的源代碼文件。該文件格式由 Microsoft 推出，與 .NET Framework 一起使用，它提供了用於編寫代碼的低級編程語言，這些代碼經過編譯以生成 EXE 或 DLL 形式的最終輸出文件。這些可以使用 Microsoft Visual Studio 創建和編譯。 Microsoft Visual Studio Express 也可用於創建和更新此類文件，它是一個免費的 IDE。 CS 文件用於應用程序開發，範圍從簡單的桌面應用程序到更複雜的程序。使用 C# 語言創建的簡單 Visual Studio 項目解決方案可以包含一個或多個此類文件。標記為包含在編譯中的文件列在 CSPROJ 文件中，該文件是項目的一部分，並告訴編譯器使用標記的文件。
