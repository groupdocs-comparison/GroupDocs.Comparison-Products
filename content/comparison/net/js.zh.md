...
############################# Head ############################
head_title: "在 .NET 中比较两个 JS 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 JS 文件。检索 JS 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 JS 文件"
description: ".NET 文档比较 API 检测两个版本的 JS 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 JS 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 JS 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.js"))
        {
            ...
            comparer.Compare("result.js"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.js")))
        {
            ...
            comparer.Compare(File.Create("result.js")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    ...
    title: "比较 JS 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 JS 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-js"
          title: "关于 JS 文件格式"
          content: |
            JS (JavaScript) 是包含在网页上执行的 JavaScript 代码的文件。 JavaScript 文件以 .js 扩展名存储。在 HTML 文档中，您可以使用标签嵌入 JavaScript 代码或包含 JS 文件。与 CSS 文件类似，JS 文件可以包含在多个 HTML 文档中以实现代码的可重用性。 JavaScript 可用于操作 HTML DOM。
