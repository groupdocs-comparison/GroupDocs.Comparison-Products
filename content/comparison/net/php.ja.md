...
############################# Head ############################
head_title: ".NETの2つのPHPファイルを比較する|ドキュメント比較API"
head_description: "3つ以上を比較してマージします（C＃.NETアプリケーションのPHPファイル。PHPファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの違いの概要を取得します."

############################# Header ############################
title: "C＃.NETでPHPファイルを比較する"
description: ".2つのバージョンのPHPファイル間の変更を検出し、比較されたドキュメント間の違いの詳細な要約を含む最終的なドキュメントにエクスポートするためのNETドキュメント比較API."
...
steps:
    enable: true
    title_left: "C＃でPHPファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/ja/comparison/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数のPHPファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.php"))
        {
            ...
            comparer.Compare("result.php"); // 指定された名前の結果ファイルを作成する
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.php")))
        {
            ...
            comparer.Compare(File.Create("result.php")); // 指定された名前の結果ファイルを作成する
        }
        ```
...
demos:
    enable: true
    title: "PHPファイルの比較のライブデモ"
    content: |
        [GroupDocs.Comparisonライブデモ](https://products.groupdocs.app/comparison/family)サイトにアクセスして、PHPファイル間の違いを今すぐ検出してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-php"
          title: "PHPファイル形式について"
          content: |
            拡張子が.phpのファイルは、サーバー上で実行されるサーバー側スクリプトを作成するために使用されるオープンソースプログラミング言語を指します。これは、大規模なWebアプリケーションの開発に広く使用されている最も人気のある強力なWebスクリプト言語です。 PHPは、ウェブ上で最大のブログシステム、つまりWordPressの中核であり、最大のソーシャルネットワークであるFacebookを運営しています。 Windows、Linux、Unix、Mac OS Xなどのさまざまなプラットフォームで動作し、現在使用されているほぼすべてのサーバー（Apache、IISなど）と互換性があります。PHPは、MySQLを含む幅広いデータベースをサポートしています。
