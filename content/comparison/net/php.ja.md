...
############################# Head ############################
head_title: ".NET で 2 つの PHP ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の PHP ファイルを比較およびマージします。 PHP ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での PHP ファイルの比較"
description: ".NET ドキュメント比較 API。PHP ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で PHP ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の PHP ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.php"))
        {
            ...
            comparer.Compare("result.php"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.php")))
        {
            ...
            comparer.Compare(File.Create("result.php")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "PHP ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ PHP ファイル間の違いを検出してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-php"
          title: "PHP ファイル形式について"
          content: |
            .php 拡張子を持つファイルは、サーバー上で実行されるサーバー側スクリプトを作成するために使用されるオープンソース プログラミング言語を指します。これは、大規模な Web アプリケーションの開発に広く使用されている、最も人気があり強力な Web スクリプト言語です。 PHP は Web 上で最大のブログ システム、つまり WordPress の中核であり、最大のソーシャル ネットワークである Facebook を運営しています。 Windows、Linux、Unix、Mac OS X などのさまざまなプラットフォームで実行され、現在使用されているほとんどすべてのサーバー (Apache、IIS など) と互換性があります。PHP は、MySQL を含む幅広いデータベースをサポートしています。
