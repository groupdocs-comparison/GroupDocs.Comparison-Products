...
############################# Head ############################
head_title: "比較 .NET 中的兩個 PHP 文件 |文檔比較 API"
head_description: "在 C# .NET 應用程序中比較並合併兩個以上的 PHP 文件。檢索 PHP 文件、圖像和文檔格式的內容、文本和样式的差異摘要。"

############################# Header ############################
title: "比較 C# .NET 中的 PHP 文件"
description: ".NET 文檔比較 API 用於檢測兩個版本的 PHP 文件之間的更改，並導出到最終文檔，其中包含所比較文檔之間差異的詳細摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比較 PHP 文件的步驟"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 開發人員可以通過實施幾個簡單的步驟輕鬆比較和合併應用程序中的多個 PHP 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.php"))
        {
            ...
            comparer.Compare("result.php"); // 創建指定名稱的結果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.php")))
        {
            ...
            comparer.Compare(File.Create("result.php")); // 創建指定名稱的結果文件
        }
        ```
...
demos:
    enable: true
    title: "比較 PHP 文件的現場演示"
    content: |
        立即訪問 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 網站檢測 PHP 文件之間的差異。
        現場演示有以下好處
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-php"
          title: "關於 PHP 文件格式"
          content: |
            擴展名為 .php 的文件是指開源編程語言，用於編寫要在服務器上執行的服務器端腳本。它是最流行、最強大的 Web 腳本語言，廣泛用於開發大型 Web 應用程序。 PHP 是網絡上最大的博客系統 WordPress 的核心，並運行著最大的社交網絡 Facebook。它運行在各種平台上，如 Windows、Linux、Unix、Mac OS X 等，並且與當今使用的幾乎所有服務器（即 Apache、IIS 等）兼容。PHP 支持廣泛的數據庫，包括 MySQL。
