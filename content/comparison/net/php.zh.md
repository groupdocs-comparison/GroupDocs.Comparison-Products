...
############################# Head ############################
head_title: "在 .NET 中比较两个 PHP 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 PHP 文件。检索 PHP 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 PHP 文件"
description: ".NET 文档比较 API 检测两个版本的 PHP 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 PHP 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 PHP 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.php"))
        {
            ...
            comparer.Compare("result.php"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.php")))
        {
            ...
            comparer.Compare(File.Create("result.php")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 PHP 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 PHP 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-php"
          title: "关于 PHP 文件格式"
          content: |
            扩展名为 .php 的文件是指用于编写要在服务器上执行的服务器端脚本的开源编程语言。它是最流行和最强大的网络脚本语言，广泛用于开发大型网络应用程序。 PHP 是网络上最大的博客系统（即 WordPress）的核心，并运行着最大的社交网络 Facebook。它运行在各种平台上，如 Windows、Linux、Unix、Mac OS X 等，并与当今使用的几乎所有服务器兼容，即 Apache、IIS 等。PHP 支持广泛的数据库，包括 MySQL。
