...
############################# Head ############################
head_title: ".NET에서 두 개의 DCM 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 DCM 파일을 비교하고 병합합니다. DCM 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET의 DCM 파일 비교"
description: ".NET 문서 비교 API는 DCM 파일의 두 버전 간의 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    enable: true
    title_left: "C#에서 DCM 파일을 비교하는 단계"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 DCM 파일을 쉽게 비교하고 병합할 수 있습니다.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.dcm"))
        {
            ...
            comparer.Compare("result.dcm"); // 지정된 이름으로 결과 파일 생성
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.dcm")))
        {
            ...
            comparer.Compare(File.Create("result.dcm")); // 지정된 이름으로 결과 파일 생성
        }
        ```
...
demos:
    enable: true
    title: "DCM 파일 비교에 대한 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DCM 파일 간의 차이점을 찾아보세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dcm"
          title: "DCM 파일 형식 정보"
          content: |
            .DCM 확장자를 가진 파일은 MRI, CT 스캔, 초음파 이미지와 같은 환자의 의료 정보를 저장하는 디지털 이미지를 나타냅니다. 이는 NEMA(National Electrical Manufacturer Association)에서 개발했으며 의료 영상의 배포 및 보기를 위한 영상 파일 형식을 표준화하기 위한 것입니다. DICOM 파일 형식과 유사하며 참조용 환자 정보를 포함할 수 있습니다.
