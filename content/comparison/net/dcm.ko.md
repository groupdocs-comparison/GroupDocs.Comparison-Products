...
############################# Head ############################
head_title: ".NET에서 두 DCM 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 DCM 파일을 비교하고 병합합니다. DCM 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET에서 DCM 파일 비교"
description: ".NET 문서 비교 API는 DCM 파일의 두 버전 간 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 DCM 파일을 쉽게 비교 및 ​​병합할 수 있습니다.
...
demos:
    enable: true
    title: "DCM 파일 비교 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 DCM 파일 간의 차이점을 감지하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dcm"
          title: "DCM 파일 형식 정보"
          content: |
            확장자가 .DCM인 파일은 MRI, CT, 초음파 영상 등 환자의 의료정보를 저장하는 디지털 영상을 의미합니다. NEMA(National Electrical Manufacturers Association)에서 개발했으며 의료 이미지 배포 및 보기를 위한 이미지 파일 형식을 표준화하기 위한 것입니다. DICOM 파일 형식과 유사하며 참고용으로 환자의 정보를 포함할 수 있습니다.
