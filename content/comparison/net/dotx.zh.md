...
############################# Head ############################
head_title: "在 .NET 中比较两个 DOTX 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 DOTX 文件。检索 DOTX 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 DOTX 文件"
description: ".NET 文档比较 API 检测两个版本的 DOTX 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 DOTX 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 DOTX 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.dotx"))
        {
            ...
            comparer.Compare("result.dotx"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.dotx")))
        {
            ...
            comparer.Compare(File.Create("result.dotx")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    enable: true
    title: "比较 DOTX 文件的现场演示"
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 DOTX 文件之间的差异。
        现场演示有以下好处
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-dotx"
          title: "关于 DOTX 文件格式"
          content: |
            带有 DOTX 扩展名的文件是由 Microsoft Word 创建的模板文件，具有用于生成更多 DOCX 文件的预格式化设置。创建一个模板文件是为了具有特定的用户设置，这些设置应该应用于从这些文件创建的后续苍蝇。这些设置包括页边距、边框、页眉、页脚和其他页面设置。此类模板用于公司信头和标准化表格等官方文件。 DOTX 文件格式随着 Microsoft Office 2007 的发布而引入，以取代二进制 DOT 文件格式，但也受到更高版本的支持。 Microsoft Word 默认打开基于 normal.dot 文件的每个新文档。如果修改，所有创建的新文件将产生与模板文件相同的设置。在 Microsoft Word 2007 中，DOT 文件格式已替换为基于 Office OpenXML 的 DOTX 文件格式。
