...
############################# Head ############################
head_title: "Comparez deux fichiers LOG dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers LOG dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents LOG."

############################# Header ############################
title: "Comparez les fichiers LOG en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers LOG et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers LOG en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers LOG dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.log"))
        {
            ...
            comparer.Compare("result.log"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.log")))
        {
            ...
            comparer.Compare(File.Create("result.log")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers LOG"
    content: |
        Détectez dès maintenant les différences entre les fichiers LOG en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
