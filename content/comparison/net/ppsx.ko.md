...
############################# Head ############################
head_title: ".NET에서 두 PPSX 파일 비교 | 문서 비교 API"
head_description: "C# .NET 애플리케이션에서 두 개 이상의 PPSX 파일을 비교하고 병합합니다. PPSX 파일, 이미지 및 문서 형식의 콘텐츠, 텍스트 및 스타일의 차이점 요약을 검색합니다."

############################# Header ############################
title: "C# .NET에서 PPSX 파일 비교"
description: ".NET 문서 비교 API는 PPSX 파일의 두 버전 간 변경 사항을 감지하고 비교된 문서 간의 차이점에 대한 자세한 요약과 함께 최종 문서로 내보냅니다."
...
steps:
    ...
    content_left: |
        [GroupDocs.Comparison](/comparison/net/)을 사용하면 .NET 개발자가 몇 가지 간단한 단계를 구현하여 애플리케이션에서 여러 PPSX 파일을 쉽게 비교 및 ​​병합할 수 있습니다.
...
demos:
    enable: true
    title: "PPSX 파일 비교 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 웹사이트를 방문하여 PPSX 파일 간의 차이점을 감지하세요.
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-ppsx"
          title: "PPSX 파일 형식 정보"
          content: |
            PPSX, Power Point Slide Show, 파일은 슬라이드 쇼 목적으로 Microsoft PowerPoint 2007 이상을 사용하여 생성됩니다. Microsoft PowerPoint 97-2003 버전에서 지원했던 PPS 파일 형식에 대한 업데이트입니다. PPSX 파일을 다른 사용자와 공유하고 열면 편집 가능한 모드로 열리는 PPTX 파일과 달리 PowerPoint 쇼로 시작됩니다. 슬라이드 쇼의 순서는 원본 프레젠테이션과 동일합니다. 모든 슬라이드는 이미지, 사운드 및 기타 내장된 미디어와 함께 제공되며 슬라이드쇼가 진행되는 동안 프레젠테이션 슬라이드와 함께 PPSX로 전송됩니다.
