...
############################# Head ############################
head_title: "在 .NET 中比较两个 MOBI 文件 |文档比较 API"
head_description: "在 C# .NET 应用程序中比较和合并两个以上的 MOBI 文件。检索 MOBI 文件、图像和文档格式的内容、文本和样式的差异摘要。"

############################# Header ############################
title: "在 C# .NET 中比较 MOBI 文件"
description: ".NET 文档比较 API 检测两个版本的 MOBI 文件之间的变化，并导出到最终文档，其中包含比较文档之间差异的详细摘要。"
...
steps:
    enable: true
    title_left: "在 C# 中比较 MOBI 文件的步骤"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) 使 .NET 开发人员可以通过实施几个简单的步骤轻松地比较和合并应用程序中的多个 MOBI 文件。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.mobi"))
        {
            ...
            comparer.Compare("result.mobi"); // 创建指定名称的结果文件
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.mobi")))
        {
            ...
            comparer.Compare(File.Create("result.mobi")); // 创建指定名称的结果文件
        }
        ```
...
demos:
    ...
    content: |
        通过访问 [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) 网站立即检测 MOBI 文件之间的差异。
        现场演示有以下好处
