...
############################# Head ############################
head_title: "Comparez deux fichiers PPT dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers PPT dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents PPT."

############################# Header ############################
title: "Comparez les fichiers PPT en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers PPT et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers PPT en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers PPT dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.ppt"))
        {
            ...
            comparer.Compare("result.ppt"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.ppt")))
        {
            ...
            comparer.Compare(File.Create("result.ppt")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers PPT"
    content: |
        Détectez dès maintenant les différences entre les fichiers PPT en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-ppt"
          ...
          content: |
            Un fichier avec l'extension PPT représente un fichier PowerPoint composé d'une collection de diapositives à afficher sous forme de diaporama. Il spécifie le format de fichier binaire utilisé par Microsoft PowerPoint 97-2003. Un fichier PPT peut contenir plusieurs types d'informations différents, tels que du texte, des puces, des images, du multimédia et d'autres objets OLE intégrés. Microsoft a proposé à partir de 2007 un nouveau format de fichier pour PowerPoint, connu sous le nom de PPTX, basé sur Office OpenXML et différent de ce format de fichier binaire. Plusieurs autres programmes d'application tels que OpenOffice Impress et Apple Keynote peuvent également créer des fichiers PPT.
