...
############################# Head ############################
head_title: "Comparez deux fichiers PPSX dans .NET | API de comparaison de documents"
head_description: "Comparez et fusionnez plus de deux fichiers PPSX dans des applications C# .NET. Récupérez le résumé des différences dans le contenu, le texte et le style des fichiers, images et formats de documents PPSX."

############################# Header ############################
title: "Comparez les fichiers PPSX en C# .NET"
description: "API de comparaison de documents .NET pour détecter les modifications entre deux versions de fichiers PPSX et exporter vers un document final avec un résumé détaillé des différences entre les documents comparés."
...
steps:
    enable: true
    title_left: "Étapes pour comparer les fichiers PPSX en C#"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) permet aux développeurs .NET de comparer et de fusionner facilement plusieurs fichiers PPSX dans leurs applications en implémentant quelques étapes simples.
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.ppsx"))
        {
            ...
            comparer.Compare("result.ppsx"); // Créer un fichier de résultats avec le nom spécifié
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.ppsx")))
        {
            ...
            comparer.Compare(File.Create("result.ppsx")); // Créer un fichier de résultats avec le nom spécifié
        }
        ```
...
demos:
    enable: true
    title: "Démonstrations en direct de comparaison de fichiers PPSX"
    content: |
        Détectez dès maintenant les différences entre les fichiers PPSX en visitant le site Web [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family).
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-ppsx"
          ...
          content: |
            PPSX, Power Point Slide Show, les fichiers sont créés à l'aide de Microsoft PowerPoint 2007 et supérieur à des fins de diaporama. Il s'agit d'une mise à jour du format de fichier PPS pris en charge par les versions Microsoft PowerPoint 97-2003. Lorsqu'un fichier PPSX est partagé avec un autre utilisateur et ouvert, il démarre sous forme d'affichage PowerPoint contrairement au fichier PPTX qui s'ouvre en mode modifiable. La séquence du diaporama est la même que dans la présentation originale. Toutes les diapositives accompagnent les images, les sons et autres médias intégrés accompagnent les diapositives de présentation au PPSX pendant le diaporama.
