...
############################# Head ############################
head_title: ".NET で 2 つの XLSM ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の XLSM ファイルを比較およびマージします。 XLSM ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での XLSM ファイルの比較"
description: ".NET ドキュメント比較 API。XLSM ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で XLSM ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の XLSM ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.xlsm"))
        {
            ...
            comparer.Compare("result.xlsm"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.xlsm")))
        {
            ...
            comparer.Compare(File.Create("result.xlsm")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "XLSM ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ XLSM ファイル間の違いを検出してください。
