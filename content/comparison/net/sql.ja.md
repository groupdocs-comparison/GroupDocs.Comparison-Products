...
############################# Head ############################
head_title: ".NET で 2 つの SQL ファイルを比較する |ドキュメント比較 API"
head_description: "C# .NET アプリケーションで 3 つ以上の SQL ファイルを比較およびマージします。 SQL ファイル、画像、ドキュメント形式のコンテンツ、テキスト、スタイルの相違点の概要を取得します。"

############################# Header ############################
title: "C# .NET での SQL ファイルの比較"
description: ".NET ドキュメント比較 API。SQL ファイルの 2 つのバージョン間の変更を検出し、比較されたドキュメント間の相違点の詳細な概要を含む最終ドキュメントにエクスポートします。"
...
steps:
    enable: true
    title_left: "C# で SQL ファイルを比較する手順"
    content_left: |
        [GroupDocs.Comparison](/comparison/net/) を使用すると、.NET 開発者は、いくつかの簡単な手順を実装することで、アプリケーション内の複数の SQL ファイルを簡単に比較およびマージできます。
    ...
    code: |
        ...
        using (Comparer comparer = new Comparer("source.sql"))
        {
            ...
            comparer.Compare("result.sql"); // 指定された名前で結果ファイルを作成します
        }
        ...
        using (Comparer comparer = new Comparer(File.OpenRead("source.sql")))
        {
            ...
            comparer.Compare(File.Create("result.sql")); // 指定された名前で結果ファイルを作成します
        }
        ```
...
demos:
    enable: true
    title: "SQL ファイルを比較するライブ デモ"
    content: |
        [GroupDocs.Comparison Live Demos](https://products.groupdocs.app/comparison/family) Web サイトにアクセスして、今すぐ SQL ファイル間の違いを検出してください。
...
about_formats:
    ...
    format:
        ...
        - icon: "far fa-file-sql"
          title: "SQL ファイル形式について"
          content: |
            拡張子が .sql のファイルは、リレーショナル データベースを操作するコードが含まれる構造化照会言語 (SQL) ファイルです。これは、データベースに対する CRUD (作成、読み取り、更新、および削除) 操作の SQL ステートメントを作成するために使用されます。 SQL ファイルは、デスクトップおよび Web ベースのデータベースを操作するときによく使用されます。 Java Persistence Query Language (JPQL)、LINQ、HTSQL、4D QL、その他いくつかの SQL の代替手段がいくつかあります。 SQL ファイルは、Microsoft SQL Server、MySQL のクエリ エディター、および Windows OS 上のメモ帳などの他のプレーン テキスト エディターで開くことができます。
